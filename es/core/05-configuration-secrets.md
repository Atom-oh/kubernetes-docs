...
En Kubernetes, la gestión de la configuración es una parte importante de administrar los ajustes de las aplicaciones por separado del código. En este capítulo, exploraremos en detalle los métodos de gestión de configuración de Kubernetes, incluidos ConfigMaps, Secrets, variables de entorno y el montaje de configuración mediante volúmenes.
...
> **Concepto clave**: ConfigMaps almacenan datos de configuración en pares clave-valor, separando el código de la aplicación de la configuración.

ConfigMaps son objetos de API que almacenan datos de configuración en pares clave-valor. Usar ConfigMaps te permite separar los datos de configuración de las imágenes de contenedor, lo que hace que las aplicaciones sean más portables.
...
ConfigMaps se pueden crear de varias maneras:
...
ConfigMaps se pueden usar de las siguientes maneras:
...
ConfigMaps se pueden usar en Pods de las siguientes maneras:
...
Secrets son objetos de API que almacenan información sensible, como contraseñas, tokens OAuth y claves SSH. Secrets son similares a ConfigMaps, pero proporcionan características de seguridad adicionales para almacenar datos sensibles.
...
- **kubernetes.io/service-account-token**: Almacena tokens de service account.
...
Secrets se pueden crear de varias maneras:
...
Secrets se pueden usar en Pods de las siguientes maneras:
...
Secrets están codificados en base64 de forma predeterminada, pero esto no es cifrado. Para mejorar la seguridad de los secrets, considera los siguientes métodos:

1. **Cifrado de etcd**: Cifra los secrets almacenados en etcd.
2. **RBAC**: Restringe el acceso a secrets.
3. **Network Policies**: Limita los Pods que pueden acceder a secrets.
...
La Downward API te permite exponer información del Pod y del contenedor como variables de entorno.
...
Montar archivos de configuración en contenedores mediante volúmenes proporciona un método de gestión de configuración más flexible que las variables de entorno.
...
Considera las siguientes mejores prácticas al gestionar configuración en Kubernetes:

### 1. Separate Configuration from Code

Gestiona el código de la aplicación y la configuración por separado. Esto elimina la necesidad de reconstruir la aplicación al cambiar la configuración.

### 2. Environment-Specific Configuration Management

Gestiona la configuración por separado para diferentes entornos, como desarrollo, pruebas y producción. Puedes usar namespaces para separar entornos y usar diferentes ConfigMaps y Secrets para cada entorno.
...
Usa siempre Secrets para almacenar información sensible, como contraseñas, API keys y certificados. Usa ConfigMaps solo para datos de configuración no sensibles.

### 4. Maintain Immutability

Al cambiar la configuración, crea una versión nueva en lugar de modificar la existente. Esto facilita los rollbacks y permite hacer seguimiento del historial de cambios de configuración.
...
La configuración usada como variables de entorno requiere reiniciar el Pod para actualizarse. Usa Deployments para realizar actualizaciones progresivas.
...
Valida la configuración antes de aplicarla. Una configuración inválida puede causar fallos en la aplicación.

### 7. Document Configuration

Documenta las opciones de configuración y sus efectos. Esto ayuda a los miembros del equipo a comprender y gestionar la configuración.

## Configuration Management in Amazon EKS

En Amazon EKS, puedes usar los distintos servicios de AWS además de las características básicas de gestión de configuración de Kubernetes para gestionar configuración y secrets. Esta sección cubre varias formas de gestionar configuración en EKS y la integración con servicios de AWS.
...
External Secrets Operator necesita permisos de IAM adecuados para acceder a AWS Secrets Manager. Puedes usar IRSA para asociar roles de IAM con service accounts de Kubernetes.
...
AWS AppConfig es un servicio que gestiona y despliega configuración de aplicaciones. Usar AppConfig te permite actualizar configuración dinámicamente sin volver a desplegar aplicaciones.
...
Usar EKS Fargate te permite ejecutar Kubernetes Pods sin gestionar nodos. Puedes configurar el entorno de ejecución del Pod mediante Fargate profiles.
...
Kubernetes secrets están codificados en base64 de forma predeterminada, lo cual no es cifrado. Puedes usar AWS KMS (Key Management Service) para cifrar secrets en tu cluster de EKS.
...
Usar IRSA (IAM Roles for Service Accounts) para asociar roles de IAM con service accounts de Kubernetes permite que los Pods accedan de forma segura a los servicios de AWS.
...
Considera las siguientes mejores prácticas al gestionar configuración en EKS:

1. **Use IRSA**: Usa siempre IRSA para conceder permisos mínimos a Pods al acceder a servicios de AWS.

2. **Encrypt Secrets**: Usa KMS para cifrar secrets en tu cluster de EKS.
...
5. **Environment-Specific Configuration Separation**: Gestiona la configuración por separado para entornos de desarrollo, pruebas y producción. Usa namespaces de Kubernetes y etiquetas de recursos de AWS.

6. **Minimize IAM Policies**: Sigue el principio de privilegio mínimo al acceder a servicios de AWS.
...
ACK es una herramienta que te permite gestionar recursos de AWS desde Kubernetes. Usando ACK, puedes crear y gestionar recursos de AWS mediante manifests de Kubernetes.
...
eksctl es una herramienta de línea de comandos para crear y gestionar clusters de EKS. Puedes usar eksctl para gestionar la configuración del cluster.
...
AWS CDK (Cloud Development Kit) es una herramienta para definir recursos de AWS usando lenguajes de programación. Puedes usar CDK para definir clusters de EKS y recursos relacionados.
...
En este capítulo, aprendimos sobre los métodos de gestión de configuración de Kubernetes. ConfigMaps y Secrets proporcionan formas básicas de gestionar la configuración de aplicaciones, y puedes pasar esta configuración a los contenedores mediante variables de entorno y volúmenes. También cubrimos las mejores prácticas de gestión de configuración y las herramientas externas de gestión de configuración.

En entornos de Amazon EKS, puedes lograr una gestión de configuración más potente y segura usando servicios de AWS junto con las características básicas de gestión de configuración de Kubernetes. Puedes gestionar secrets de forma segura integrando servicios como AWS Secrets Manager, Parameter Store, KMS e IAM, y conceder permisos mínimos a Pods mediante IRSA. Además, puedes actualizar dinámicamente la configuración sin volver a desplegar aplicaciones usando AWS AppConfig.

La gestión eficaz de configuración es importante para mejorar la mantenibilidad, escalabilidad y seguridad de las aplicaciones de Kubernetes. Es importante elegir la estrategia de gestión de configuración adecuada para los requisitos de tu aplicación y seguir las mejores prácticas. En entornos de EKS, puedes crear soluciones de gestión de configuración más potentes mediante la integración con servicios de AWS.

En el próximo capítulo, aprenderemos sobre la seguridad de Kubernetes.

## Quiz

Para comprobar lo que aprendiste en este capítulo, intenta resolver el [cuestionario de configuración y secrets](../quizzes/core/05-configuration-secrets-quiz.md).
...
- [Documentación oficial de Kubernetes - Distribuir credenciales de forma segura usando Secrets](https://kubernetes.io/docs/tasks/inject-data-application/distribute-credentials-secure/)
