...
eBPF es una tecnología revolucionaria que permite que programas aislados se ejecuten dentro del kernel de Linux. Este documento cubre todo, desde los conceptos básicos de eBPF hasta aplicaciones prácticas en entornos Kubernetes.
...
### Prerequisites
- Kernel Linux 4.18 o superior (se recomienda 5.10+)
- bpftool, bcc-tools
- Clúster Kubernetes (opcional)
...
**eBPF (extended Berkeley Packet Filter)** es una tecnología que permite que programas definidos por el usuario se ejecuten de forma segura dentro del kernel de Linux. Diseñado originalmente para filtrar paquetes de red como BPF, se ha ampliado y ahora se usa en diversas áreas, incluyendo redes, seguridad, trazado y análisis de rendimiento.

> **Concepto clave**: eBPF te permite extender y observar el comportamiento del kernel sin modificar el código fuente del kernel ni cargar módulos del kernel.
...
- Dedicado a la captura y filtrado de paquetes de red
...
- Soporte para diversos puntos de enganche
...
1. **Extensión de funcionalidades sin modificar el kernel**: Extiende las funcionalidades del kernel sin cambiar el código fuente del kernel
...
- Mejora de rendimiento de 4 a 5 veces respecto al intérprete
- Ejecución directa como instrucciones nativas de CPU
- Optimizaciones específicas de arquitectura aplicadas

### 2.4 eBPF Maps

Los eBPF maps son estructuras de datos para compartir datos entre el kernel y user space, y para almacenar estado.
...
LSM BPF aplica dinámicamente políticas de seguridad.
...
bpftool es la herramienta oficial para administrar programas y maps de eBPF.
...
bpftrace es un lenguaje de trazado de alto nivel con estilo DTrace.
...
libbpf es una biblioteca C para cargar programas eBPF y soporta CO-RE (Compile Once, Run Everywhere).

**Beneficios de CO-RE**:
- Ejecuta programas eBPF compilados en diversas versiones del kernel
- Reubicación de structs usando BTF (BPF Type Format)
- Dependencias reducidas de headers del kernel
...
Cilium aplica Network Policies L3/L4/L7 usando eBPF.
...
- Soporte para Direct Server Return (DSR)
...
eBPF permite una observación profunda del comportamiento del sistema y de las aplicaciones. A diferencia del monitoreo tradicional basado en agentes, eBPF recopila datos a nivel del kernel, proporcionando información más rica con menor sobrecarga.
...
- Generación automática de mapas de Services
- Trazado distribuido
- Perfilado de CPU
- Logging dinámico
...
- Generación automática de mapas de dependencias
- Monitoreo de SLO
...
Kepler (Kubernetes-based Efficient Power Level Exporter) usa eBPF para monitorear el consumo de energía de los contenedores.
...
LSM BPF combina Linux Security Module con eBPF para aplicar dinámicamente políticas de seguridad.
...
**Solución alternativa para el límite de tamaño de stack**:
...
Aunque eBPF es una herramienta potente, existen riesgos de seguridad:

- **Filtración de información**: Puede acceder a datos sensibles
- **Ataques DoS**: Puede causar degradación del rendimiento
- **Escalada de privilegios**: Pueden existir vulnerabilidades por mala configuración
...
Para verificar tu comprensión de este documento, intenta el siguiente cuestionario:

- [Cuestionario sobre fundamentos de eBPF](../quizzes/basics/05-ebpf-fundamentals-quiz.md)
...
Checklist práctica para aprender eBPF:
...
eBPF es una tecnología revolucionaria que permite extender y observar de forma segura el comportamiento del kernel de Linux. Aquí tienes un resumen del contenido clave cubierto en este documento:
...
8. **Limitaciones**: Considera el tamaño de stack, los bucles y la compatibilidad de versiones del kernel
