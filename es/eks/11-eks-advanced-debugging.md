...
> **Versiones compatibles**: EKS 1.28+, kubectl 1.28+
> **Última actualización**: February 23, 2026

Para el funcionamiento estable de clusters de Amazon EKS, son esenciales un framework sistemático de respuesta a incidentes y habilidades avanzadas de debugging. Este documento proporciona una guía práctica para diagnosticar y resolver rápidamente problemas complejos que ocurren en entornos de producción.
...
Los primeros 5 minutos son los más críticos cuando ocurre un incidente. Sigue esta checklist en orden.
...
| Severidad | Clasificación | Alcance del impacto | Tiempo de respuesta | Ejemplos |
|----------|---------------|--------------|---------------|----------|
| **P1** | Crítica | Interrupción completa del Service | Dentro de 15 minutos | Falla del Control Plane, todos los Nodes NotReady |
| **P2** | Alta | Falla de funcionalidad importante | Dentro de 1 hora | Falla completa de un workload específico, problemas de conectividad de red |
| **P3** | Media | Impacto parcial | Dentro de 4 horas | Algunos reinicios de Pod, degradación del rendimiento |
| **P4** | Baja | Problemas menores | Dentro de 24 horas | Retraso en recopilación de logs, alertas de monitoreo no críticas |
...
EKS envía 5 tipos de logs de Control Plane a CloudWatch Logs.

| Tipo de log | Descripción | Casos de uso principales |
|----------|-------------|-------------------|
| **api** | Logs del API server | Trazado de llamadas API, análisis de errores |
| **audit** | Audit logs | Auditorías de seguridad, seguimiento de cambios |
| **authenticator** | Logs de autenticación IAM | Debugging de fallas de autenticación |
| **controllerManager** | Logs de controller manager | Problemas de reconciliación de recursos |
| **scheduler** | Logs del scheduler | Problemas de colocación de Pod |
...
|---|-------|---------|------------|
| 1 | Discordancia del bootstrap script | El Node no aparece en el cluster | Verificar que la versión de AMI coincida con la versión del cluster |
| 2 | Configuración incorrecta de security group | Falla la comunicación Node-Control Plane | Comprobar reglas inbound para los puertos 443, 10250 |
| 3 | Problema de configuración de DNS en VPC | Falla la resolución DNS | Habilitar enableDnsHostnames, enableDnsSupport |
| 4 | Permisos insuficientes del IAM role | Falla la autenticación | Verificar que las políticas requeridas estén adjuntas al node role |
| 5 | Faltan tags de subnet | Falla el aprovisionamiento de Node | Comprobar el tag kubernetes.io/cluster/\<name\> |
| 6 | Subnet privada sin NAT | Falla el image pull | Configurar NAT Gateway o VPC endpoints |
| 7 | Instance profile no adjunto | Falla el lanzamiento de EC2 | Comprobar la configuración de Launch Template |
| 8 | Error en user data script | Bootstrap interrumpido | Comprobar /var/log/cloud-init-output.log |
...
|------------|-------------|------------|
| AccessDenied | Permisos IAM insuficientes | Comprobar las políticas del node role |
| AsgInstanceLaunchFailures | Falló el lanzamiento de instance en ASG | Comprobar Launch Template, capacidad de subnet |
| ClusterUnreachable | No se puede conectar al cluster | Comprobar VPC endpoints, security groups |
| InsufficientFreeAddresses | Escasez de direcciones IP | Ampliar el CIDR de subnet o añadir nuevas subnets |
| NodeCreationFailure | Falló la creación del Node | Comprobar límites del servicio EC2, disponibilidad de AMI |
...
| Nivel | Descripción | Objetivo de MTTD | Capacidades clave |
|-------|-------------|-------------|------------------|
| **Level 1** | Básico | 30 minutos | Alertas de métricas básicas, búsqueda manual de logs |
| **Level 2** | Reactivo | 15 minutos | Alertas de umbral, alertas basadas en logs, dashboards básicos |
| **Level 3** | Proactivo | 5 minutos | Anomaly detection, composite alarms, runbooks automatizados |
| **Level 4** | Predictivo | 2 minutos | Predicción basada en ML, auto-remediation, chaos engineering |
...
| Severidad | Slack | PagerDuty | Email | SMS | Auto-Remediation |
|----------|-------|-----------|-------|-----|------------------|
| **P1 Critical** | #incidents | Inmediato | Team Lead | On-call | Sí |
| **P2 High** | #alerts-high | retraso de 15min | Team | - | Condicional |
| **P3 Medium** | #alerts | - | Team | - | No |
| **P4 Low** | #alerts-low | - | Resumen diario | - | No |
...
|---------|-------|------------|
| **CrashLoopBackOff** | Crash de la aplicación, comando inválido, dependencias faltantes | `kubectl logs --previous`, revisar el código/configuración de la aplicación |
| **ImagePullBackOff** | Imagen no encontrada, tag incorrecto, falla de autenticación | Verificar el nombre de la imagen, revisar `imagePullSecrets` |
| **OOMKilled** | Límite de memoria excedido | Aumentar el límite de memoria, corregir memory leak |
| **CreateContainerConfigError** | ConfigMap/Secret faltante, referencia inválida | `kubectl describe pod`, verificar que existan los recursos referenciados |
| **Pending (resources)** | No hay Node con suficiente CPU/memoria | Escalar Nodes, ajustar resource requests |
| **Pending (scheduling)** | Discordancia de nodeSelector, affinity, taint | Comprobar la sección Events en `kubectl describe pod` |
| **ContainerCreating (delayed)** | Falla de montaje de volume, problema del network plugin | Comprobar estado de PVC, estado de CNI pod |
| **ErrImagePull** | No se puede conectar al image registry | Comprobar conectividad de red, ECR endpoints |
| **RunContainerError** | Configuración inválida del container, problema de securityContext | `kubectl describe pod`, revisar securityContext |
| **PostStartHookError** | Falló el hook postStart | Revisar el comando del hook, ajustar timeout |
| **PreStopHookError** | Falló el hook preStop | Revisar el comando del hook, ajustar terminationGracePeriodSeconds |
| **FailedScheduling** | Escasez de recursos, PVC binding pendiente | Comprobar recursos del Node, estado de PVC |
| **FailedMount** | Falló el montaje de volume, problema del CSI driver | Comprobar logs del CSI driver, estado de PV/PVC |
| **NetworkNotReady** | CNI plugin no listo | Comprobar estado de aws-node pod, CNI logs |
| **NodeNotReady** | Problema de kubelet, desconexión de red | Comprobar kubelet logs, estado del Node |
| **Evicted** | Presión de recursos del Node (disco, memoria) | Limpiar recursos del Node, ajustar límites de recursos |
| **BackOff** | Estado de retry backoff | Comprobar logs de errores anteriores, resolver la causa raíz |
| **InvalidImageName** | Formato de nombre de imagen inválido | Verificar la sintaxis del nombre de imagen |
...
| Herramienta | Propósito | Instalación/Uso |
|------|---------|-------------------|
| **netshoot** | Network debugging | `kubectl run net --image=nicolaka/netshoot -it --rm` |
| **eks-node-viewer** | Visualización de recursos de Node | `go install github.com/awslabs/eks-node-viewer/cmd/eks-node-viewer@latest` |
| **crictl** | Debugging de container runtime | En el Node: `sudo crictl ps`, `sudo crictl logs` |
| **kubeval** | Validación de YAML | `kubeval deployment.yaml` |
| **stern** | Logging de múltiples Pods | `stern <pod-pattern> -n <namespace>` |
| **k9s** | Gestión de cluster en TUI | `k9s -n <namespace>` |
| **kubectx/kubens** | Cambio de context/namespace | `kubectx <context>`, `kubens <namespace>` |
...
- Información del sistema (OS, kernel, memoria, CPU)
- kubelet logs y configuración
- containerd logs y configuración
- Logs del CNI plugin
- Configuración de red (iptables, routing)
- Uso de disco
...
Para probar tu comprensión del contenido cubierto en este documento, intenta el [EKS Advanced Debugging Quiz](../quizzes/eks/11-eks-advanced-debugging-quiz.md).

### Next Document

Para aprender a integrar clusters de EKS con entornos on-premises, consulta [EKS Hybrid Nodes](../eks-hybrid-nodes/README.md).
