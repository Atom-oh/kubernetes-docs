...
La gestión de secrets en Kubernetes es un pilar de la seguridad de las aplicaciones. Este documento cubre varias herramientas de gestión de secrets, desde Secrets nativos hasta External Secrets Operator, Sealed Secrets, HashiCorp Vault y SOPS.
...
Kubernetes Secrets son objetos que almacenan información sensible, como contraseñas, tokens y claves.
...
External Secrets Operator conecta sistemas externos de gestión de secretos con Kubernetes para sincronizar Secrets automáticamente.
...
Sealed Secrets es una herramienta que te permite almacenar de forma segura Secrets cifrados en Git.
...
SOPS es una herramienta para gestionar archivos cifrados que admite varios backends de cifrado, incluidos Age, PGP y AWS KMS.
...
La gestión de secrets en Kubernetes es un pilar de la seguridad:

1. **Native Secrets**: Simples, pero solo proporcionan codificación Base64
2. **External Secrets Operator**: Sincroniza con almacenes de secretos externos
3. **Sealed Secrets**: Almacena de forma segura secretos cifrados en Git
4. **HashiCorp Vault**: Gestión centralizada de secretos de nivel empresarial
5. **SOPS**: Cifrado basado en archivos, compatible con GitOps

### Key Recommendations

- No uses Native Secrets por sí solos en producción
- Nunca hagas commit de secretos en Git en texto plano
- Implementa rotación automática
- Aplica el principio de privilegio mínimo
- Habilita auditoría de acceso a secrets
