...
Este cuestionario evalúa tu comprensión de los conceptos de seguridad de Kubernetes, incluyendo autenticación, autorización, NetworkPolicy, Security Context y gestión de Secret.
...
3. ¿Qué NO se puede configurar mediante el Security Context de un pod en Kubernetes?
   - A) ID de usuario (UID) del Container
   - B) ID de grupo (GID) del Container
   - C) NetworkPolicy del Container
   - D) Capacidad de escalamiento de privilegios del Container
   
<details>
<summary>Mostrar respuesta</summary>

**Respuesta: C) NetworkPolicy del Container**

**Explicación:**
Security Context define configuraciones de privilegios y control de acceso a nivel de pod o container. Esto incluye ID de usuario (runAsUser), ID de grupo (runAsGroup), capacidad de escalamiento de privilegios (allowPrivilegeEscalation), containers privilegiados (privileged) y capabilities. Sin embargo, las network policies se definen mediante recursos NetworkPolicy separados, no mediante Security Context.
</details>

4. ¿Cuál es el propósito principal de un ServiceAccount en Kubernetes?
   - A) Autenticación para usuarios fuera del cluster
   - B) Proporcionar identidad para que los pods se comuniquen con el API server
   - C) Cifrar la comunicación entre nodes
   - D) Otorgar privilegios de administrador del cluster
   
<details>
<summary>Mostrar respuesta</summary>

**Respuesta: B) Proporcionar identidad para que los pods se comuniquen con el API server**

**Explicación:**
Los service accounts proporcionan identidad a los procesos que se ejecutan dentro de pods para comunicarse con el Kubernetes API server. Cada namespace tiene una service account predeterminada, y los pods usan esta service account predeterminada a menos que se especifique explícitamente otra. Los service accounts pueden usarse con RBAC para limitar qué operaciones pueden realizar los pods.
</details>
...
6. ¿Cuál es el más restrictivo de los tres niveles de políticas en Pod Security Standards?
   - A) Privileged
   - B) Baseline
   - C) Restricted
   - D) Enforced
...
Pod Security Standards define tres niveles de políticas:
  - Privileged: Sin restricciones, se permiten todos los privilegios
  - Baseline: Previene rutas conocidas de escalamiento de privilegios
  - Restricted: La política más restrictiva, con configuraciones de seguridad reforzadas aplicadas

La política Restricted es la más restrictiva, sigue el principio de mínimo privilegio y aplica buenas prácticas de seguridad. Esta política prohíbe containers privilegiados, el uso compartido de host namespace, montajes de host path y más.
</details>

7. ¿Cuál es el método más efectivo para proteger datos de Secret en Kubernetes?
...
**Explicación:**
Los datos de Secret en Kubernetes se almacenan codificados en Base64 por defecto, pero esto es una codificación simple, no cifrado. Usar la configuración de cifrado de etcd cifra los datos de Secret antes de almacenarlos en etcd, protegiendo la información sensible frente al acceso no autorizado a la base de datos etcd. El aislamiento por namespace y los labels pueden ayudar con el control de acceso, pero no protegen los datos en sí.
</details>

8. ¿Cuál NO es un método para mejorar la seguridad de container images en Kubernetes?
   - A) Escaneo de vulnerabilidades de images
   - B) Uso de registries confiables
   - C) Firma y verificación de images
   - D) Otorgar privilegios root a los containers
   
<details>
<summary>Mostrar respuesta</summary>

**Respuesta: D) Otorgar privilegios root a los containers**

**Explicación:**
Otorgar privilegios root a los containers debilita la seguridad. Los métodos para mejorar la seguridad de container images incluyen escaneo de vulnerabilidades de images, uso de registries confiables, firma y verificación de images, aplicación del principio de mínimo privilegio, eliminación de paquetes innecesarios y ejecución de containers como usuarios non-root.
</details>

9. ¿Cuál es el propósito principal de Audit Logging en Kubernetes?
   - A) Recopilar logs de pods
   - B) Registrar solicitudes al API server
   - C) Monitorear el estado de nodes
...
10. ¿Cuál NO es una característica de los containers privilegiados en Kubernetes?
    ...
    - B) Uso de la pila de red del host
...
**Explicación:**
Los containers privilegiados pueden acceder a casi todas las capabilities del host, pero no tienen acceso automático a recursos de Kubernetes en otros namespaces. El acceso entre namespaces está controlado por permisos RBAC. Los containers privilegiados pueden acceder a dispositivos del host, pila de red, módulos del kernel, etc., lo que representa riesgos de seguridad significativos, por lo que deben usarse solo cuando sea absolutamente necesario.
</details>
...
2. Explica tres métodos para aplicar el 'principio de mínimo privilegio' en Kubernetes.

<details>
<summary>Mostrar respuesta</summary>

**Respuesta:**
1. Usar RBAC para otorgar solo los permisos mínimos necesarios
2. Ejecutar containers como usuarios non-root en el security context del pod
3. Usar network policies para permitir solo la comunicación necesaria
4. Restringir el uso de containers privilegiados
5. Restringir las capabilities de containers
...
**Respuesta:**
Secret se usa para almacenar información sensible (contraseñas, tokens, claves, etc.), mientras que ConfigMap se usa para almacenar datos de configuración generales. Los Secrets se almacenan codificados en Base64 (no cifrados por defecto), pueden configurarse para montarse solo en memoria y se referencian solo cuando se crean los pods. Sin embargo, sin configuración adicional, ambos se almacenan en texto plano en etcd, por lo que se requieren configuraciones de cifrado de etcd para una seguridad completa.
</details>
...
4. gVisor: Aislamiento adicional mediante implementación de kernel en espacio de usuario
5. Kata Containers: Aislamiento a nivel de hardware usando VMs ligeras
6. Firecracker: Aislamiento basado en MicroVM

Estas tecnologías limitan el impacto que los containers pueden tener sobre el sistema host y reducen el riesgo de ataques de escape de containers.
</details>
...
3. Crea un pod con los siguientes requisitos de security context:
   ...
   - No se permite el escalamiento de privilegios
   - Filesystem root de solo lectura
...
4. Crea un Secret y móntalo como un volume en un pod con los siguientes requisitos:
   - Secret llamado 'db-credentials'
   - Contiene 'username=admin' y 'password=s3cr3t'
   - Montado en la ruta '/etc/db-credentials' dentro del pod
...
1. Proporciona tres ejemplos de políticas que pueden implementarse usando OPA (Open Policy Agent) Gatekeeper en Kubernetes y explícalas.
...
OPA Gatekeeper es una herramienta potente para aplicar políticas a clusters de Kubernetes. Estos son ejemplos de políticas que pueden aplicarse:

1. **Restricción de Image Registry**: Obligar a que las images solo se descarguen desde registries aprobados, evitando el uso de images de fuentes no confiables.
   ```yaml
...
2. **Prevenir containers privilegiados**: Prohibir el uso de containers privilegiados para reducir riesgos de seguridad.
   ```yaml
   apiVersion: constraints.gatekeeper.sh/v1beta1
...
3. **Aplicar Resource Limits**: Obligar a que todos los containers tengan establecidos limits de CPU y memory para prevenir ataques de agotamiento de recursos.
   ```yaml
   apiVersion: constraints.gatekeeper.sh/v1beta1
...
OPA Gatekeeper también puede aplicar otras políticas variadas, como exigir labels específicos en namespaces, restringir hostnames de ingress, limitar tipos de volume y prevenir montajes de host path.
</details>
...
mTLS (mutual TLS) es un método en el que tanto el cliente como el server se autentican mutuamente usando certificados. Aquí se muestra cómo implementar mTLS en Kubernetes y sus beneficios:

**Métodos de implementación:**

1. **Uso de Service Mesh**: Service meshes como Istio y Linkerd implementan mTLS automáticamente mediante sidecar proxies.
   ```yaml
   # Istio example
...
3. **Gestión de certificados**: Usa herramientas como cert-manager para gestionar el ciclo de vida de certificados.
   ```yaml
...
1. **Autenticación mutua**: El cliente y el server se autentican mutuamente, evitando ataques man-in-the-middle
2. **Comunicación cifrada**: Todo el tráfico service-to-service se cifra, evitando escuchas no autorizadas
3. **Control de acceso detallado**: La identidad basada en certificados permite un control de acceso detallado
4. **Arquitectura Zero Trust**: Requiere autenticación para toda comunicación, independientemente de la ubicación de red
...
2. **Generación y verificación de Software Bill of Materials (SBOM)**:
   ...
   - Rastrear todos los componentes de software incluidos en images
   ```bash
...
3. **Escaneo de vulnerabilidades**:
   ...
   - Integrar el escaneo en pipelines CI/CD
   ```bash
...
5. **Aplicar Image Policies**:
   - Aplicar políticas basadas en la antigüedad de la image, gravedad de vulnerabilidades, origen del registry, etc.
   ```yaml
   apiVersion: constraints.gatekeeper.sh/v1beta1
...
6. **Supply Chain Levels for Software Artifacts (SLSA)**:
   - Rastrear la procedencia del build
...
Combinar estos métodos ayuda a proteger los entornos Kubernetes contra ataques a la software supply chain.
</details>
...
El zero trust security model se basa en el principio de "never trust, always verify". Este es el enfoque para implementar zero trust en Kubernetes:

1. **Identity and Access Management detallado**:
   - Implementar políticas RBAC sólidas
   - Otorgar los permisos mínimos necesarios a service accounts
...
2. **Segmentación de red**:
   ...
   - Aplicar network policies que solo permitan la comunicación explícitamente requerida
   - Implementar microsegmentation
   ```yaml
   apiVersion: networking.k8s.io/v1
   kind: NetworkPolicy
   metadata:
...
5. **Cifrado**:
   ...
   - Integración de herramientas de gestión de Secret (Vault, Sealed Secrets, etc.)

6. **Detección y respuesta ante amenazas**:
   - Desplegar herramientas de monitoreo de runtime security
   - Detectar comportamiento anómalo y generar alertas
   - Implementar mecanismos de respuesta automatizada

7. **Configuración de workloads con mínimo privilegio**:
   - Ejecutar containers como usuarios non-root
...
El modelo zero trust no es una solución única, sino un enfoque que combina varias capas de seguridad, basado en la verificación continua y el principio de mínimo privilegio.
</details>
...
1. **Falco**:
   ...
   - **Características**: 
     - Opera a nivel de kernel para monitorear la actividad interna de containers
...
2. **Seccomp (Secure Computing Mode)**:
   - **Cómo funciona**: Restringe las llamadas al sistema que pueden usar los containers
   - **Características**:
     - Usa una función integrada del kernel de Linux
     - Solo pueden ejecutarse las llamadas al sistema permitidas
...
3. **AppArmor**:
   ...
   - **Características**:
     - Control de acceso detallado para files, network, capabilities, etc.
     - Incluido por defecto en distribuciones Linux
...
4. **SELinux**:
   - **Cómo funciona**: Aplica políticas de Mandatory Access Control (MAC)
   - **Características**:
     - Políticas de seguridad detalladas basadas en labels
     - Soporte de estándar de seguridad de grado militar
...
5. **OPA Gatekeeper**:
   - **Cómo funciona**: Gobernanza de runtime basada en políticas
   - **Características**:
     - Definición declarativa de políticas
     - Amplio alcance de aplicación de políticas
     - Soporte para modos de auditoría y enforcement

6. **Herramientas comerciales (Aqua, Sysdig, StackRox, etc.)**:
   - **Cómo funciona**: Proporcionan plataformas completas de seguridad de containers
   - **Características**:
     - Escaneo de vulnerabilidades integrado, protección en runtime, monitoreo de cumplimiento
     - Detección de anomalías basada en machine learning
     - Funciones de dashboard y reportes

7. **gVisor**:
   - **Cómo funciona**: Proporciona un kernel en espacio de usuario entre la aplicación y el kernel del host
   - **Características**:
     - Capa de aislamiento adicional entre container y host
     - Intercepción y emulación de llamadas al sistema
     - Existe sobrecarga de rendimiento
   - **Ejemplo de implementación**:
     ```yaml
...
8. **Kata Containers**:
   ...
   - **Características**:
     ...
     - Mayor sobrecarga que los containers normales

**Comparación y criterios de selección**:
  - **Nivel de seguridad**: Kata Containers y gVisor proporcionan el aislamiento más fuerte
  - **Impacto en rendimiento**: Seccomp tiene una sobrecarga mínima, Kata Containers tiene la mayor
  - **Complejidad de implementación**: Seccomp y AppArmor son relativamente fáciles, SELinux es complejo
  - **Monitoreo vs prevención**: Falco es principalmente monitoreo, otras herramientas proporcionan protección preventiva
  - **Facilidad de integración**: OPA Gatekeeper se integra estrechamente con Kubernetes

Elige la combinación de herramientas adecuada según los requisitos de seguridad de tu organización, los objetivos de rendimiento y la tolerancia a la complejidad operativa.
</details>
