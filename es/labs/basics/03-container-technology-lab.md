...
- Optimizar imágenes usando multi-stage builds
...
## Ejercicio 1: Escritura de Dockerfile y construcción de imagen

### Objetivo
Containerizar una aplicación web sencilla.
...
**Paso 1.2: Escribir el Dockerfile**
```bash
cat > Dockerfile << 'EOF'
FROM nginx:alpine
...
- Las imágenes basadas en `alpine` son pequeñas, lo que es ventajoso para despliegues en K8s
...
### Objetivo
Ejecutar un contenedor y depurar sus componentes internos.
...
**Paso 2.3: Conectarse a los componentes internos del contenedor**
```bash
...
- `-it` en `docker exec -it` corresponde a las opciones interactive + TTY
...
### Objetivo
Usar multi-stage builds para optimizar el tamaño de la imagen.
...
**Paso 3.3: Comparar tamaños de imagen**
```bash
...
- En los multi-stage builds, usa `FROM ... AS builder` para nombrar la etapa de build
- Usa `COPY --from=builder` para copiar solo artefactos de la etapa anterior
