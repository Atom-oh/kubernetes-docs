...
Cilium Service Mesh のトラフィック管理は、eBPF ベースの L4 ロードバランシングと Envoy ベースの L7 ルーティングを組み合わせます。この章では、CiliumEnvoyConfig、CiliumNetworkPolicy の L7 ルール、Gateway API 統合を通じて高度なトラフィック管理機能を説明します。
...
CiliumEnvoyConfig は特定の Service 向けの Envoy 設定を定義します。
...
CiliumNetworkPolicy は L7 レベルでのきめ細かなトラフィック制御を可能にします。
...
eBPF ベースの L4 ロードバランシングは kube-proxy を置き換えます。
...
L7 ロードバランシングは Envoy を通じて提供されます。
...
- [Security](./03-security.md): mTLS と L7 NetworkPolicy を設定する
- [Observability](./04-observability.md): Hubble でトラフィックを監視する
- [Ingress & Gateway](./05-ingress-gateway.md): 外部トラフィック管理

## 参考資料

- [Cilium L7 Policy ドキュメント](https://docs.cilium.io/en/stable/security/policy/language/#layer-7-examples)
- [CiliumEnvoyConfig リファレンス](https://docs.cilium.io/en/stable/network/servicemesh/envoy-config/)
