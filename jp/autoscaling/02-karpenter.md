...
Karpenter は、Kubernetes クラスターの Node プロビジョニングを自動化するオープンソースのクラスターオートスケーラーです。Karpenter はワークロード要件に基づいて適切なコンピューティングリソースを動的にプロビジョニングし、アプリケーションの可用性を確保するとともにクラスターの効率を最適化します。

### Karpenter の主な利点

1. **高速スケーリング**: ワークロード要件に基づき、数秒以内に Node をプロビジョニング
...
Karpenter v1.13（2026 年 6 月リリース）以降、Karpenter は Kubernetes Dynamic Resource Allocation（DRA）に基づくデバイス割り当ての追跡をサポートしています。Karpenter は GPU や特殊なアクセラレーターなどの claim ベースのリソースを認識してプロビジョニングの判断に反映できるようになり、`nvidia.com/gpu` のような拡張リソースだけでなく、DRA の `ResourceClaim`/`DeviceClass` オブジェクトを使用する AI/HPC ワークロードに対しても正確なスケーリングを実現します。DRA ベースの追跡には Kubernetes 1.29 以降が必要です。

### Node 有効期限の設定

Node 有効期限の設定では、Karpenter が Node を削除するタイミングを定義します。
...
Karpenter v1.13 で追加された NodeReadinessController は、不要なスケジューリングのブロックを減らすため、readiness 関連の Taint（Node の初期化中に適用されるものなど）を自動的に無視します。これにより、以前は `startupTaints` による手動処理が必要だった初期化遅延の問題が緩和され、新しい Node が Ready になる間のスケジューリングの安定性とプロビジョニングの信頼性が向上します。

### 2026 年 7 月の更新: v1.14 リリース

2026 年 7 月 11 日にリリースされた Karpenter v1.14 には、次の内容が含まれます。

- **CapacityBuffers API のサポート**: 突発的なスケールアウトの急増を吸収するための余剰キャパシティを宣言的に予約
- **Preview インスタンスタイプのサポート**: まだ一般提供されていないインスタンスタイプをプロビジョニングに選択可能
- **Nitro Enclaves のサポート**: launch template に `EnclaveOptions.Enabled` を設定可能。機密コンピューティングワークロードに有用
- バグ修正: secondary ENI の primary IP の考慮、Zonal Shift cache の確実な hydration、AWS SDK client timeout の operator config への接続など

詳細は [v1.14.0 release notes](https://github.com/aws/karpenter-provider-aws/releases/tag/v1.14.0) を参照してください。

続いて 2026 年 7 月 17 日には、保守されているすべての minor line を対象に、協調した patch release（v1.3.8 から v1.11.3）がリリースされ、それぞれで upstream の `sigs.k8s.io/karpenter` バージョンが更新されました。古い line を使用している場合は、その line の最新 patch への更新を推奨します（[release list](https://github.com/aws/karpenter-provider-aws/releases)）。

2026 年 7 月 22 日、AWS はさらに、Karpenter（および EKS Auto Mode）の Node pool 向けに Elastic Fabric Adapter（EFA）network device 設定と EC2 placement group のサポートを発表しました。EFA 対応インスタンスの network interface は、VPC IP address を消費しない EFA-only、または標準 ENI に設定できます。また、cluster/spread/partition の placement strategy を Node pool 設定で直接指定できます。これは分散トレーニングおよび推論ワークロードに有用です。[発表](https://aws.amazon.com/about-aws/whats-new/2026/07/amazon-eks-efa-placement-groups/) を参照してください。

### 2026 年 8 月の更新: v1.14.1 Patch Release

v1.14 line で最初の patch である [v1.14.1](https://github.com/aws/karpenter-provider-aws/releases/tag/v1.14.1) は、2026 年 8 月 21 日に公開されました。これは upstream の `sigs.k8s.io/karpenter` バージョンを更新し、v1.14.0 以降に作成された修正を cherry-pick するメンテナンスリリースです。
...
次の図は、Karpenter の Node consolidation プロセスを示しています。この機能は、クラスター効率の最適化とコスト削減に重要です。
...
4. **Node Consolidation**: より効率的な Node 構成に統合
...
Karpenter は、アプリケーションの可用性を確保するために PDB を尊重します。
...
Karpenter は、さまざまな Kubernetes およびクラウドサービスと統合されます。
...
Karpenter は、コストを最適化するために EC2 Spot instances をサポートしています。
...
Karpenter がクラスターリソースを識別できるよう、Tag を設定します。
...
Karpenter は EKS Fargate と併用して、ハイブリッドクラスターを構成できます。
...
Karpenter は Amazon ARC（Application Recovery Controller）の Zonal Shift をサポートしています。Availability Zone（AZ）で障害が発生すると、Karpenter はその AZ での新しい Node のプロビジョニングを自動的に停止し、代わりに正常な AZ にワークロードをスケジュールします。AWS が AZ の正常性を自動検出し、トラフィックシフトと復旧を処理する Zonal Autoshift もサポートされています。

障害が検出されると、Karpenter は outage 中の不要な Node 置換によってクラスターがさらに不安定になることを防ぐため、任意の disruption（consolidation、drift handling など）も自動的に停止します。これは既存の EKS ARC リソースを直接使用するもので、custom resource は不要です。また、`ENABLE_ZONAL_SHIFT` option で有効化します。
...
1. **適切なインスタンスタイプを選択**: ワークロードに適したインスタンスタイプを選択
2. **多様なインスタンスタイプを許可**: 可用性とコスト最適化のために、さまざまなインスタンスタイプを許可
3. **適切な TTL を設定**: ワークロードパターンに一致する TTL を設定
4. **Node Consolidation を有効化**: リソース使用率を最適化するために Node consolidation を有効化
...
1. **Spot Instances を活用**: コスト削減のために Spot instances を使用
2. **適切なインスタンスサイズを選択**: ワークロードに適したインスタンスサイズを選択
3. **ゼロスケーリングを活用**: アクティビティがない場合は Node 数を 0 に削減
4. **Node の有効期限を設定**: 定期的な Node 置換により最新のインスタンスタイプを活用
...
1. **複数の Availability Zone を使用**: 複数の Availability Zone に Node をデプロイ
2. **On-demand と Spot Instances を混在**: 可用性とコストのバランスを確保
3. **適切な PDB を設定**: アプリケーションの可用性を確保
4. **中断処理を最適化**: Node 中断時のワークロード可用性を確保
...
**解決策**:
- Provisioner requirements を確認
- Pod resource request を検証
- Availability Zone の制約を確認
...
Karpenter は、Kubernetes クラスターの Node プロビジョニングを自動化する強力なオートスケーラーです。ワークロード要件に基づいて適切なコンピューティングリソースを動的にプロビジョニングし、アプリケーションの可用性を確保するとともにクラスターの効率を最適化します。
...
Karpenter を使用すると、クラスター管理を簡素化し、リソース使用率を最適化して、コストを削減できます。特に Amazon EKS のようなクラウド管理 Kubernetes 環境では、Karpenter の利点を最大限に活用できます。

### 次のステップ

- Karpenter を使用してコスト最適化戦略を実装する
...
この章で学んだ内容を確認するには、[トピッククイズ](../quizzes/autoscaling/06-karpenter-quiz.md) を試してください。
