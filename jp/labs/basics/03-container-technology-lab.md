...
- [ ] [コンテナ技術](../../basics/03-container-technology.md) の学習を完了済み
...
- `docker build -t name:tag .` では、`.` はビルドコンテキストディレクトリです
- `alpine` ベースのイメージは小さく、K8s デプロイに有利です
- キャッシュを使用せずにビルドするには `docker build --no-cache` を使用します
</details>
...
- `docker exec -it` の `-it` は interactive + TTY オプションを表します
- 詳細情報を表示するには `docker inspect container-name` を使用します
...
- マルチステージビルドでは、ビルドステージに名前を付けるために `FROM ... AS builder` を使用します
- 前のステージから成果物だけをコピーするには `COPY --from=builder` を使用します
- 最終イメージにはビルドツールが含まれないため、サイズが大幅に削減されます
</details>
...
- [Pods and Workloads ラボ](../core/02-pods-and-workloads-lab.md)
