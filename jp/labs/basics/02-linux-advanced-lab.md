...
- [ ] [Linux 運用スキル](../../basics/02-linux-advanced.md) の学習を完了済み
...
### 目標
Kubernetes の kubectl 出力に似た JSON データを jq で処理します。

### 手順

**ステップ 1.1: サンプル JSON を作成**
```bash
...
- `select(condition)` は条件に一致する項目のみをフィルタリングします
- `@tsv` はタブ区切り形式で出力します
...
### 目標
K8s 運用に役立つ簡単なシェルスクリプトを作成します。

### 手順

**ステップ 2.1: ヘルスチェック スクリプト**
```bash
...
- `$((RANDOM % N))` は 0 から N-1 までの乱数を生成します
- `grep -oP` は Perl 正規表現を使用して一致した部分のみを抽出します
