...
Amazon EKS Auto Mode は、ワークロード要件に基づいてノードを自動的にプロビジョニングおよび最適化することで、Kubernetes ノード管理を完全に自動化する機能です。このガイドでは、EKS Auto Mode の概念、設定方法、本番環境におけるベストプラクティスについて説明します。

### 2026年7月アップデート: ARC Zonal Shift のサポート

2026年7月10日以降、EKS Auto Mode クラスターは Amazon Application Recovery Controller (ARC) の zonal shift および autoshift をサポートしています。Auto Mode がコンピューティングを管理するため、フラグの設定や Karpenter バージョンの管理を行わずに zonal shift サポートを利用できます。クラスターで ARC zonal shift を有効にするだけです。zonal shift が有効化されると、Auto Mode は影響を受けた AZ での新しいキャパシティのプロビジョニングを停止し、そのゾーンのノードに対する consolidation や drift などの任意の中断を停止します。追加料金はかかりません。詳細は、[発表](https://aws.amazon.com/about-aws/whats-new/2026/07/eks-auto-mode-arc-zonal-shift)および [ARC zonal shift ドキュメント](https://docs.aws.amazon.com/eks/latest/userguide/zone-shift.html)を参照してください。

## 目次

1. [Auto Mode の開始方法](./01-getting-started.md) - クラスターの作成と Auto Mode の有効化
...
4. [Spot Instance 活用戦略](./04-spot-strategies.md) - 混合キャパシティと中断への対応
5. [運用と管理](./05-operations.md) - disruption budget、ローリング置換、モニタリング
6. [コスト管理と最適化](./06-cost-management.md) - コスト分析、Spot の節約、right-sizing
7. [Node ライフサイクル管理](./07-node-lifecycle.md) - 有効期限、AMI 管理、最新性ポリシー
...
EKS Auto Mode は、AWS が管理する完全自動化されたノード管理ソリューションです。内部的に Karpenter をベースとしており、ユーザーが個別のノード管理コンポーネントをインストールまたは設定する必要なく、AWS がすべてを管理します。
...
| スケーリング方式 | Cluster Autoscaler | Pod ごと | Karpenter ベース |
| スケーリング速度 | 数分 | 即時（Pod スケジュール） | 数十秒 |
| Instance Type の選択 | 事前定義 | 自動 | 自動最適化 |
| Spot サポート | 手動設定 | 非対応 | 自動管理 |
| GPU ワークロード | 対応 | 制限あり | 完全対応 |
| DaemonSet サポート | 対応 | 非対応 | 対応 |
| コスト最適化 | 手動 | 中程度 | 自動 |
| 複雑さ | 高 | 低 | 低 |
| カスタマイズ性 | 高 | 低 | 中 |
...
EKS Auto Mode は Karpenter をベースに動作しますが、AWS 管理のコントロールプレーン内で実行されます。
...
| 項目 | 制限 |
|------|-------|
| クラスターあたりの最大 NodePool 数 | 100 |
| NodePool あたりの最大ノード数 | 1000 |
| クラスターあたりの最大ノード数 | 5000 |
| 最小 EKS バージョン | 1.29 |
| サポートされる AMI ファミリー | AL2023, Bottlerocket |
| Windows ノード | 非対応 |
...
EKS Auto Mode の設定が完了したら、以下のトピックを学ぶことをお勧めします。
...
学習内容を確認するには、[EKS Auto Mode クイズ](../quizzes/eks-auto-mode/01-getting-started-quiz.md)に挑戦してください。
