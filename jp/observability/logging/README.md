...
Kubernetes 環境における効果的なロギングは、システムの可視性、トラブルシューティング、セキュリティ監査に不可欠です。このドキュメントでは、ロギングの基礎、ログ収集パイプラインのアーキテクチャ、EKS 環境向けのロギング戦略を扱います。
...
5. [ソリューションの比較](./#solution-comparison)
...
構造化ロギングは、一貫した形式でログメッセージを出力するため、解析と分析が容易になります。非構造化テキストログとは異なり、構造化ログはフィールド値のペアで構成され、より効率的な検索とフィルタリングを可能にします。
...
| **自動化**           | 解析なしで分析ツールにすぐ使用可能      |
| **アラート設定**  | 特定フィールド値に基づくアラートルールを容易に作成 |

### ログレベル

ログレベルはメッセージの重要度と深刻度を示します。ログレベルを適切に使用することは、効果的なトラブルシューティングとノイズの削減に不可欠です。
...
| **WARN**  | 3      | 潜在的な問題の状況             | リトライの発生、パフォーマンス低下           |
| **ERROR** | 4      | エラーの発生（回復可能）             | API 呼び出し失敗、バリデーション失敗                 |
| **FATAL** | 5      | 致命的エラー（回復不可能）           | Service の起動失敗、必須依存関係の欠落 |
...
| **Sidecar**     | アプリケーションごとの分離、カスタム処理 | リソースオーバーヘッド                 | 特殊なログ形式、マルチテナント |
| **Direct Push** | リアルタイムで柔軟な配信                 | アプリケーションの変更が必要 | 高パフォーマンス要件     |
...
処理済みログを保存してインデックス化します。ストレージ方式はソリューションの特性によって異なります。
...
コンテナの標準出力/標準エラー出力を介したロギングは、Kubernetes のデフォルトパターンです。
...
* Kubernetes ネイティブのアプローチ
* 自動ログローテーション管理（`/var/log/containers/`）
* `kubectl logs` コマンドが利用可能
* 個別のボリュームマウントは不要
...
ファイルベースのロギングや特殊な処理が必要な場合に使用します。
...
### EKS Control Plane ロギング

EKS control plane ログは CloudWatch Logs に送信されます。
...
| --------------------------- | ---------- | -------------- | -------------- | -------------- |
| **インストールの複雑さ** | 低        | 中         | なし（マネージド） | 高           |
| **クエリ言語**          | LogQL      | Lucene/DQL     | Insights QL    | SQL            |
| **全文検索**        | 限定的    | 優れている      | 良好           | 良好           |
| **スキーマ**                  | スキーマレス | スキーマレス     | スキーマレス     | スキーマ定義済み |
| **圧縮**             | 高       | 中         | N/A            | 非常に高      |
| **リアルタイムテーリング**       | サポート済み  | サポート済み      | 限定的        | サポート済み      |
| **アラート**                | Grafana    | 組み込み       | 組み込み       | Grafana        |
| **マルチテナント**           | サポート済み  | サポート済み      | サポート済み      | サポート済み      |
...
> **注記**: 実際のコストは、クエリパターン、保持期間、リージョンによって大きく異なる場合があります。
