...
Dynatrace は、AI を活用したフルスタックのオブザーバビリティプラットフォームです。OneAgent テクノロジーにより、アプリケーション、インフラストラクチャ、ユーザーエクスペリエンスを自動的に監視し、Davis AI エンジンが問題の根本原因を自動分析します。

## 主な機能

| 機能 | 説明 |
|---------|-------------|
| **OneAgent** | 単一エージェントによるフルスタック監視 |
| **自動インストルメンテーション** | コード変更なしの自動トレーシング |
| **Davis AI** | AI を活用した根本原因分析 |
| **PurePath** | 分散トレーシング技術 |
| **Smartscape** | リアルタイムトポロジーマッピング |
| **フルスタック** | インフラストラクチャからユーザーエクスペリエンスまで |
...
Dynatrace コンソールで、次の権限を持つ API Token を作成します。
...
| 種類 | 単位 | 含まれるもの |
|------|------|----------|
| **Full-Stack** | Host Unit | インフラストラクチャ + APM + Logs |
| **Infrastructure** | Host Unit | インフラストラクチャ監視のみ |
| **Application Security** | Host Unit | RASP + 脆弱性分析 |
| **DEM (Digital Experience)** | Session | RUM + Synthetic |
| **Log Monitoring** | GiB | Log の収集と分析 |
...
Dynatrace は OpenTelemetry データをネイティブに収集します。
...
[Dynatrace クイズ](../../quizzes/observability/tracing/04-dynatrace-quiz.md)で理解度を確認しましょう。
