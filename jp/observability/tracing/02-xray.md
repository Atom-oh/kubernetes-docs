...
AWS X-Ray は、分散アプリケーション内のリクエストをトレースして分析するための AWS ネイティブサービスです。EKS 環境で X-Ray を使用すると、マイクロサービス間のリクエストフローを可視化し、パフォーマンスのボトルネックを特定し、エラーの根本原因を特定できます。
...
AWS Distro for OpenTelemetry (ADOT) を使用した X-Ray 統合：
...
ServiceLens は、X-Ray トレース、CloudWatch メトリクス、ログを統合します：
