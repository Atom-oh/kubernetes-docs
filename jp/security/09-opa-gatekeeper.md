...
OPA (Open Policy Agent) Gatekeeper は、Rego 言語を使用して宣言的な policy を記述し、Admission Controller を通じて cluster policy を適用する、Kubernetes 向けの policy management tool です。
...
OPA Gatekeeper は、以下を提供する強力な policy engine です。

1. **Rego Language**: 宣言的で柔軟な policy 記述
2. **ConstraintTemplate**: 再利用可能な policy template
3. **Constraint**: 具体的な policy 適用
4. **Mutation**: 自動的な resource 変更
5. **Audit**: 既存 resource に対する policy compliance の確認
6. **Testing**: Gator による policy validation

Kyverno と比較すると learning curve は高めですが、複雑な policy logic に対してより高い柔軟性を提供します。
