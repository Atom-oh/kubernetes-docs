...
このドキュメントの例に沿って進めるには、以下の環境が必要です。
...
**eBPF (extended Berkeley Packet Filter)** は、ユーザー定義のプログラムを Linux kernel 内で安全に実行できるようにする技術です。もともとは BPF としてネットワークパケットのフィルタリング用に設計されましたが、拡張され、現在では networking、security、tracing、performance analysis など、さまざまな領域で使われています。

> **重要な概念**: eBPF を使うと、kernel source code を変更したり kernel modules をロードしたりすることなく、kernel の動作を拡張・観測できます。
...
- JIT compilation によるネイティブ性能

| 機能 | Traditional BPF | eBPF |
|---------|-----------------|------|
| Registers | 2 (32-bit) | 11 (64-bit) |
| 命令数 | 4,096 | 1 million+ |
| Map support | なし | さまざまな map types |
| Use case | Packet filtering | General-purpose kernel programming |
| Call capabilities | なし | Helper functions, BPF-to-BPF calls |
| State storage | 不可能 | maps 経由で可能 |

### 1.3 eBPF が革新的である理由

eBPF は以下の理由により革新的です。

1. **kernel 変更なしの機能拡張**: kernel source code を変更せずに kernel 機能を拡張
2. **安全な実行**: Verifier がプログラムの安全性を保証
3. **高い性能**: JIT compilation によるネイティブコードレベルの性能
4. **動的ロード**: 再起動なしでプログラムをロード/アンロード
5. **本番環境での安定性**: クラッシュや無限ループなしの安全な実行
...
| 観点 | eBPF | Kernel Module |
|--------|------|---------------|
| **安全性** | Verifier が安全性を保証 | kernel をクラッシュさせる可能性 |
| **移植性** | CO-RE により kernel version 非依存 | kernel version ごとに再コンパイルが必要 |
| **Loading** | 動的 load/unload | insmod/rmmod が必要 |
| **Privileges** | CAP_BPF または CAP_SYS_ADMIN | Root privileges が必要 |
| **Debugging** | 制限あり | 完全な kernel debugging が可能 |
| **Performance** | JIT compilation により最適化 | ネイティブ性能 |
| **Feature scope** | 指定された hook points のみ | 無制限 |
| **Development difficulty** | 比較的容易 | 高度な専門知識が必要 |
...
Verifier は eBPF の中核となるセキュリティ機構です。プログラムが kernel 内で実行される前に、以下を検証します。

**検証項目**:
- 無限ループがないこと（DAG 構造チェック）
- 境界外メモリアクセスがないこと
- 初期化されていない変数を使用していないこと
- helper function calls が正しいこと
- プログラムの終了が保証されること
...
**JIT Compilation の利点**:
- interpreter と比較して 4〜5 倍の性能向上
- native CPU instructions として直接実行
- アーキテクチャ固有の最適化を適用

### 2.4 eBPF Maps

eBPF maps は、kernel と user space の間でデータを共有し、状態を保存するためのデータ構造です。

**主な Map Types**:

| Map Type | 説明 | Use Case |
|----------|-------------|----------|
...
eBPF programs は、kernel が提供する helper functions を通じて kernel functions にアクセスします。
...
XDP は、ネットワークドライバレベルでパケットを処理する最速の方法です。
...
| Generic XDP | Software emulation | テスト用 |
...
| Performance | 最高 | 高い |
| SKB access | 不可能 | 可能 |
| 方向 | Ingress only | Both ingress and egress |
| Packet modification | 制限あり | 柔軟 |

### 3.3 Kprobes/Uprobes

Kprobes と Uprobes は function calls を動的に trace します。
...
Tracepoints は kernel 内に事前定義された静的な trace points です。
...
LSM BPF は security policies を動的に適用します。
...
Container resources と networking を制御します。
...
bpftrace は DTrace スタイルの高水準 tracing language です。
...
BCC は Python と Lua を通じて eBPF programs を書けるようにする toolkit です。
...
| Tool | 説明 |
|------|-------------|
| `execsnoop` | 新しい process executions を trace |
| `opensnoop` | file opens を trace |
| `biolatency` | Block I/O latency |
| `tcpconnect` | TCP connections を trace |
| `tcpaccept` | TCP incoming connections を trace |
| `tcpretrans` | TCP retransmissions を trace |
...
libbpf は eBPF programs をロードするための C library であり、CO-RE (Compile Once, Run Everywhere) をサポートします。

**CO-RE の利点**:
- コンパイル済み eBPF programs をさまざまな kernel versions で実行
- BTF (BPF Type Format) を用いた struct relocation
- kernel header 依存関係の削減
...
Cilium は eBPF を使用して L3/L4/L7 network policies を適用します。
...
Calico も eBPF dataplane をサポートしています。
...
| 観点 | iptables | eBPF |
|--------|----------|------|
| **Scalability** | O(n) - service count に比例 | O(1) - map lookup |
| **Latency** | rule count とともに増加 | 一定 |
| **CPU usage** | 高い | 低い |
| **Updates** | Full table rewrite | Map entry update |
| **Observability** | 制限あり | Hubble integration |
| **Memory** | rule ごとのメモリ | 最適化された map structure |
...
eBPF により、system と application の動作を深く観測できます。従来の agent-based monitoring とは異なり、eBPF は kernel level でデータを収集するため、より豊富な情報をより低い overhead で提供します。

### 6.1 Hubble: Cilium Network Observability

Hubble は Cilium に組み込まれた network observability platform です。
...
Pixie は eBPF を使用して、application code の変更なしに telemetry を自動収集します。
...
Coroot は eBPF を使用して、追加設定なしで systems を自動監視します。
...
Kepler (Kubernetes-based Efficient Power Level Exporter) は eBPF を使用して container energy consumption を監視します。
...
### 6.5 Traditional Agents と eBPF Instrumentation の比較

| 観点 | Traditional Agents | eBPF Instrumentation |
|--------|-------------------|---------------------|
...
| **Privileges** | 通常 privileges | CAP_BPF が必要 |
...
Tetragon は Cilium project が提供する eBPF-based runtime security solution です。
...
Falco は eBPF を使用して runtime anomalous behavior を検出する CNCF project です。
...
seccomp-bpf は BPF を使用して、process が実行できる system calls を制限します。
...
LSM BPF は Linux Security Module と eBPF を組み合わせて、security policies を動的に適用します。
...
| **Max instructions** | 1 million | Program complexity の制限 |
| **Max nested calls** | 8 levels | BPF-to-BPF function call depth |
| **Map entry count** | map type により異なる | memory limits に依存 |
...
eBPF verifier はプログラムの終了を保証するためにループを制限します。
...
| Privilege | 目的 |
|-----------|---------|
| `CAP_BPF` | eBPF programs のロード（kernel 5.8+） |
| `CAP_SYS_ADMIN` | 従来の eBPF privileges |
...
**Kubernetes での Privilege Configuration**:
...
eBPF は強力なツールですが、セキュリティリスクも存在します。

- **Information leakage**: sensitive data にアクセスできる可能性
- **DoS attacks**: performance degradation を引き起こす可能性
- **Privilege escalation**: misconfiguration により脆弱性が生じる可能性
...
このドキュメントの理解度を確認するには、次の quiz に取り組んでください。
...
このドキュメントに関連する高度な内容については、以下を参照してください。

| Topic | Document Link | 説明 |
|-------|---------------|-------------|
...
eBPF は、Linux kernel の動作を安全に拡張・観測できるようにする革新的な技術です。このドキュメントで扱った主な内容を以下にまとめます。

1. **eBPF Basic Concepts**: kernel 内で安全に実行される sandboxed programs
...
5. **Kubernetes Applications**: Cilium、Calico eBPF mode による高性能 networking
...
eBPF は、cloud-native environments における networking、security、observability の未来をリードする中核技術です。
