...
< [前へ: Workload 配置戦略](./06-workload-placement.md) | [目次](./README.md) | [次へ: 運用とメンテナンス](./08-operations.md) >
...
> **注記**: `shutdownGracePeriodCriticalPods` は `shutdownGracePeriod` 内である必要があります。通常の Pod には graceful termination のために `60s - 20s = 40s` が与えられます。
