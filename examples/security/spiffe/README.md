...
| `ha-values.yaml` | Three servers with shared PostgreSQL, `spire-database` Secret key `password`, `spire-database-ca` ConfigMap key `ca.crt`, verify-full TLS, and matching anti-affinity. Prepare database/DNS/CA/Secret/storage and test failover separately. |
