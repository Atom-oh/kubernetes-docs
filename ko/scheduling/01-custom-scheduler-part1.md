...
![다중 스케줄러 접근 방식에서 API 서버가 생성한 Pod가 스케줄링 큐에 쌓이고, 기본 스케줄러와 두 커스텀 스케줄러가 schedulerName이 일치하는 Pod만 골라 각 워커 노드에 바인딩하는 흐름을 보여준다.](../.gitbook/assets/ko-scheduling-01-custom-scheduler-part1-11.png)
...
![EKS 클러스터에서 API 서버가 기본 스케줄러와 커스텀 스케줄러에 Pod를 전달하고, 커스텀 스케줄러가 관리형·자체 관리형·스팟 노드 그룹에서 노드를 선택하며, 같은 Pod의 메트릭 수집기와 함께 EC2 API 및 CloudWatch와 연동되는 구조를 보여준다.](../.gitbook/assets/ko-scheduling-01-custom-scheduler-part1-12.png)
