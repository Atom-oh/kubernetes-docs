...
![NIC에서 들어온 패킷이 iptables 모드에서는 PREROUTING, FORWARD, kube-proxy 규칙, POSTROUTING 체인을 차례로 순회해 목적지 Pod에 도달하고, eBPF 모드에서는 TC Hook의 BPF 프로그램 하나가 BPF Map을 O(1)로 조회한 뒤 소켓 레벨 connect-time LB를 거쳐 kube-proxy 없이 목적지 Pod에 도달하는 두 데이터플레인을 위아래로 비교한 다이어그램](../../.gitbook/assets/ko-networking-calico-06-ebpf-dataplane-9.png)
