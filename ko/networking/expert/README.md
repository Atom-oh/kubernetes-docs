...
| 기업·데이터센터 네트워크 | 1 → 2 → 4 → 3 → 6 | Cisco 공식 학습 범위로 L2/L3·보안·가상화·자동화의 빈 영역 점검 |
| 프로토콜·네트워크 소프트웨어 | 1 → 2 → 4 → 6 | C/C++ 기초를 갖춘 뒤 전송 프로토콜·커널 구현 프로젝트 |

공통 과정은 1·2·4장입니다. 데이터센터 경로는 3장, 클라우드 경로는 5장을 추가합니다. 프로토콜/Linux 경로는 4장의 호스트 실험으로 6장의 해당 종합 평가를 진행할 수 있습니다. 선택한 범위의 평가를 마친 뒤 다른 전문화를 추가할 수 있습니다. 주당 시간을 정할 때는 읽기뿐 아니라 환경 준비·실패 원인 분석·재현·보고서 시간을 포함합니다.
