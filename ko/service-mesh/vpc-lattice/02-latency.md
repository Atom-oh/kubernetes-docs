...
여기서 실질적인 비용은 암호 연산 자체보다 **credential 획득 경로**일 수 있습니다. 임시 credential은 설정한 provider가 캐시·갱신하며 IRSA는 STS, EKS Pod Identity는 node agent와 EKS Auth를 사용합니다. 획득이나 갱신을 기다리는 요청에는 그 지연이 포함될 수 있습니다. p50·p99 또는 더 드문 sample 중 어디에 나타날지는 갱신 빈도와 workload에 달려 있으므로 provider/cache 동작과 갱신 전후 지연 분포를 기록합니다([03번 문서](./03-auth-flow.md) 참고).
