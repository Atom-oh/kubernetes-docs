...
**진단 경로**: Lattice 호출이 실패하면 Envoy sidecar log에 문제가 발생한 `169.254.171.x` 대상 요청이 있는지 확인합니다. 이것은 proxy 경유의 증거입니다. 인터셉트를 실패 원인으로 판단하기 전에 반환 오류·route/outbound policy·서명 필드 변경 여부를 함께 확인합니다. 설정된 proxy가 요청을 정상 전달할 수도 있습니다.
