...
graph TB
    ...
    subgraph "Cluster East"
        ...
        subgraph "Data Plane East"
            GW_E[Gateway<br/>인그레스]
            SVC_E[web-service]
            SVC_E_MIRROR[web-service-west<br/>미러 서비스]
...
sequenceDiagram
    ...
    Note over App,Target: 서비스 미러링 흐름
...
sequenceDiagram
    ...
    Note over Proxy_C: 대상: web-west.production

    Proxy_C->>GW_W: mTLS (인터넷/VPN 경유)
    Note over Proxy_C,GW_W: 클러스터 간 mTLS
