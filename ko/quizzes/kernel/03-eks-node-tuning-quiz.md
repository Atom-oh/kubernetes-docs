...
**설명:**
노드마다 값이 달라지면 장애 재현이 어려워집니다. 커널 버전 간에 튜너블의 이름과 위치가 바뀌므로(sysctl → debugfs 등) 6.1에서 유효했던 설정이 6.18에서 깨질 수 있습니다. 명시적 socket별 `SO_RCVBUF`/`SO_SNDBUF` 설정은 해당 socket의 자동 크기 조정을 끄지만, `tcp_rmem`/`tcp_wmem` 범위 변경 자체가 autotuning을 끄지는 않습니다. 튜닝의 전제는 측정이므로 증상을 관련 counter와 연결하고 증거가 뒷받침하는 설정만 조정합니다.
</details>
