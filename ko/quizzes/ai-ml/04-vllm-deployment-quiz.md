...
A. 늘어난 부하에 비례해 거의 16배로 늘었다
B. 거의 그대로였다(p50 5.65s → 7.52s, +33%)이고, 집계 처리량은 거의 선형으로 늘었다
C. 더 많은 요청이 vLLM의 prefill 단계를 건너뛰게 해서 오히려 줄었다
D. 동시성 16에 도달하기 전에 GPU의 KV 캐시 메모리가 고갈되어 측정할 수 없었다

<details>
<summary>정답 보기</summary>

**정답: B. 거의 그대로였다(p50 5.65s → 7.52s, +33%)이고, 집계 처리량은 거의 선형으로 늘었다**

**설명:**
이것이 continuous batching의 핵심이다. vLLM은 새 요청을 이미 실행 중인 요청 뒤에 줄 세우지 않고, 다음 스케줄러 스텝에서 배치에 합류시킨다. 그래서 GPU가 여러 시퀀스를 순차가 아니라 병렬로 처리한다. 이번 실측에서 약 100~128 토큰 응답의 p50 지연시간은 동시성 1일 때 5.65s에서 동시성 16일 때 7.52s로만 늘었지만(+33%), 집계 completion 처리량은 약 17 tokens/s에서 208 tokens/s(클라이언트 기준)로 늘었다 — GPU KV 캐시 사용률이 전체 구간에서 3% 미만이었으므로, 이 동시성 구간에서는 GPU가 메모리가 아니라 연산에 의해 제한됐기 때문이다.

**다른 옵션들의 문제점:**
- A. 이는 continuous batching이 아니라 순차(비배칭) 요청 처리에서 벌어지는 일이다.
- C. Continuous batching은 prefill을 건너뛰지 않는다. 모든 새 요청은 여전히 decode 전에 prefill을 거치며, 다른 요청들의 decode 스텝과 함께 처리될 뿐이다.
- D. 이 24 GiB L4에서 GPU KV 캐시 사용률은 동시성 16에서 최대 2.6%에 그쳤다 — 고갈과는 거리가 멀다. 이번 벤치마크는 그 한계를 찾을 만큼 동시성을 높이지 않았다.
</details>
