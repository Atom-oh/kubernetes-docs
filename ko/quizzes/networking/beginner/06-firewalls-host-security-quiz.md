...
**설명:** 좁은 allow는 추가이며 기존 권한 회수가 아닙니다. 본문은 TCP 22와 8000의 클라이언트 allow를 각각 만들어 같은 실습 NIC·목적지·포트의 deny보다 앞에 넣습니다. 따라서 SSH와 별개로 HTTP 권한만 제거할 수 있습니다. 사용자 `before.rules`도 결과에 영향을 주므로 실제 소유자와 앞선 처리를 확인해야 합니다. 경쟁 관리자를 둘 쓰면 안전해지기보다 추론이 어려워집니다.
...
**설명:** 이 UFW 절차는 firewalld처럼 규칙별 permanent·runtime을 나누지 않습니다. 위치는 바뀌지만 인터페이스·출발지·목적지·프로토콜·포트가 의도한 규칙을 식별합니다. HTTP만 고장 내는 실험에서는 독립된 TCP 8000 allow만 제거하고, 복원할 때 남은 실습 deny 앞에 삽입해야 합니다. 그 실험에서 TCP 22 allow는 제거하지 않습니다. 최초 활성화에는 관리 접근 확인도 필요합니다. NIC 설정이 그대로라고 모든 인바운드 서비스가 유지되는 것은 아닙니다.
