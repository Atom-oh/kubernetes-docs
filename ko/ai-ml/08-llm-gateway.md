...
![제안 파이프라인: 인증·파싱·라우팅·재인가·변환·예약·호출 후 출력 검사·중계·비용 계산·정산·감사 완료.](../../assets/llm-gateway-request-pipeline.svg)
...
| 2 | **파싱** | 제한된 입력을 파싱하고 RawBody는 빠른 경로 후보로만 유지합니다. 최종 정책 변환이 전달 가능 여부를 결정합니다. |
...
| 5 | **Filters** | 필수 개인정보·프롬프트 정책과 실제 송신 표현을 검사합니다. 필수 필터 실패는 차단하며 외부 분류기도 승인된 송신 대상이어야 합니다. |
| 6 | **PreCheck / 예약** | 대상별 변환 입력·출력·추론 허용량을 다시 계산하고 호출 전 쿼터·금액을 원자적으로 예약합니다. 거부 시 추론 지출은 없지만 남용 제한은 적용합니다. |
| 7 | **Provider call** | 호출 전 `request_started`를 영속 기록하고 검사된 본문을 보냅니다. API·필수 변환이 허용할 때만 원문을 보존하고 이 단계에서 제공자 자격 증명을 붙입니다. |
| 8 | **Output guard** | 텍스트·완성된 도구 인자를 버퍼링해 공개 전에 검사합니다. 크기·시간을 제한하고 검사 실패 시 차단합니다. |
| 9 | **Stream relay** | 승인된 프로토콜 이벤트를 전달하고 업스트림·사용자 관측 TTFT를 구분합니다. 확정된 응답을 재시작하지 않습니다. |
...
| **앱 / AWS SDK** | 선택한 Bedrock 런타임 API | 단순 endpoint 교체가 아닌 지원되는 AWS 호환 어댑터 | 인바운드 인증, 워크로드 신원으로 업스트림 서명, 선택한 API·이벤트 스트림 프레이밍 구현 |
...
3. **공유 정책 코어**는 요청별 인증·라우팅·재인가·변환·예약·사용량 계산을 수행하며 프로토콜별 기능·단가 규칙은 별도로 적용됩니다.
...
\* Bedrock Claude InvokeModel은 `anthropic_version: bedrock-2023-05-31`, URI의 `modelId`, AWS 인증·이벤트 스트림을 요구합니다. Messages의 모델 ID만 바꾸는 작업이 아니며 Converse의 봉투도 다릅니다. 모든 보존 항목은 라우팅·필수 변환 조건을 따릅니다.
...
클라이언트 ──요청──▶ 게이트웨이                                       제공자
                     ...
                     │    - block ⇒ 402/429; 추론 지출 미차감 (남용 제한은 적용)
                     │    - warn이면 헤더에 경고만 붙이고 통과 (block이 tie에서 이김)
                     │ ③ 쿼터와 금액 원자적 예약; 예약·요청 ID 영속 기록
                     │──────────────────────── 요청 ─────────────────▶
                     │◀─────────────── SSE 스트림 (usage 포함) ────────
...
| LLM 라우터 | 추가 모델 호출 | 토큰·요청 요금 | 결과 평가 | 라우팅 비용·개인정보 포함 |
...
- [ ] 최종 요청 변환·계산 후 금액·쿼터를 원자 예약하고 불명확한 사용량 대사
