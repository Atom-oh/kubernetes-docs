...
```mermaid
flowchart TD
    A[Experiment CRD 생성] --> B[Katib 컨트롤러]
    B --> C[선택된 알고리즘용<br/>Suggestion 서비스 생성]
    C --> D[Suggestion이<br/>하이퍼파라미터 조합 제안]
    D --> E1[Trial 1<br/>학습 잡]
    D --> E2[Trial 2<br/>학습 잡]
    D --> E3[Trial N<br/>학습 잡]
    E1 --> F[메트릭 수집 사이드카가<br/>목표 메트릭 보고]
    E2 --> F
    E3 --> F
    F --> C
    C -->|maxTrialCount 도달<br/>또는 목표값 달성| G[Experiment Succeeded 상태]
    G --> H[최적 Trial과 하이퍼파라미터를<br/>Experiment status에 기록]
```
