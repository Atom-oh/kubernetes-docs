...
```mermaid
flowchart TB
    A[클라이언트 요청] --> B[InferenceService]
    B --> C{Predictor 스펙}
    C --> D[선택: Transformer<br/>전/후처리]
    C --> E[선택: Explainer]
    D --> F{배포 모드}
    E --> F
    C --> F
    F -->|Serverless| G[Knative가 관리하는 Pod<br/>scale-to-zero 가능]
    F -->|Raw Deployment| H[일반 Deployment/Service<br/>+ HPA, scale-to-zero 없음]
    G --> I[모델 서버가 아티팩트를<br/>로드하고 추론 수행]
    H --> I
    I --> J[클라이언트에 응답]
```
