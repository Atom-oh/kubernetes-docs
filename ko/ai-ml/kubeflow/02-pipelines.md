...
```mermaid
graph LR
    A[Python 파이프라인<br/>@dsl.pipeline / @dsl.component] --> B[KFP SDK 컴파일러<br/>IR YAML 생성]
    B --> C[KFP API 서버<br/>파이프라인 저장, Run 접수]
    C --> D[백엔드가 IR YAML을<br/>Argo Workflow로 변환]
    D --> E[Argo Workflow 컨트롤러<br/>단계 스케줄링]
    E --> F[컴포넌트 Pod 실행]
    F --> G[아티팩트를 오브젝트 스토어에 기록<br/>S3 / MinIO]
    F --> H[실행 및 아티팩트 메타데이터를<br/>MLMD에 기록]
    G --> H
```
