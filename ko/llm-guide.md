...
| URL | 내용 | 용도 |
|-----|------|------|
| [llms.txt](https://www.atomai.click/kubernetes-docs/llms.txt) | 전체 문서의 제목·URL 색인 | LLM이 필요한 페이지만 골라 읽게 할 때 |
...
세 파일 모두 사이트가 배포될 때마다 자동으로 다시 생성되므로 항상 최신 콘텐츠와 일치합니다. 퀴즈(정답 포함)는 색인에는 있지만 full 파일에서는 제외됩니다 — LLM 컨텍스트에 정답지를 섞지 않기 위해서입니다.
