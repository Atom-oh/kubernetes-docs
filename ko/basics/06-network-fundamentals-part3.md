...
#### HTTP/1.1 요청과 응답 읽기 {#http11-message-structure}

HTTP 버전들은 메서드·상태 코드·필드의 의미를 공유합니다. HTTP/1.1에서는 이 개념을 **시작 줄 → 헤더 필드 줄 → 빈 줄 → 선택적 본문** 형태로 볼 수 있습니다. 본문에는 텍스트나 바이너리가 들어갈 수 있습니다. “텍스트 형식의 HTTP/1.1”은 시작 줄과 헤더를 설명하는 말이지 모든 페이로드가 텍스트라는 뜻은 아닙니다.

아래는 **설명용 HTTP/1.1 메시지이며 캡처 결과나 실행 명령이 아닙니다**. 읽기 쉽도록 화면에는 LF 줄바꿈을 표시했습니다. 실제 전송에서는 시작 줄과 각 헤더 줄을 **CRLF(`\r\n`)**로 끝내고, 추가 CRLF로 헤더 영역을 종료합니다. 두 본문은 모두 **ASCII 5바이트**인 `hello`이며 **끝에 개행이 없습니다**. 코드 블록을 닫기 위해 화면에 표시된 마지막 줄바꿈은 본문에 포함하지 않습니다.
...
| 구성 요소 | 읽는 방법 |
|---|---|
| 요청 줄 | `POST`는 메서드, `/echo`는 요청 대상(여기서는 경로), `HTTP/1.1`은 버전입니다. 필수 `Host` 필드는 대상 호스트 이름과 선택적 포트(authority)를 지정합니다. |
| 상태 줄 | `HTTP/1.1`은 버전, `200`은 상태 코드, `OK`는 선택적인 사유 문구입니다. 결과 해석에는 코드를 사용합니다. |
| 헤더 필드 | `Name: value` 줄에 메타데이터를 담으며 필드 이름은 대소문자를 구분하지 않습니다. `Content-Type`은 표현의 미디어 유형과 여기서는 문자셋을 설명합니다. |
| 빈 줄 | 헤더 영역을 끝냅니다. 뒤따르는 본문의 종료 위치까지 정하지는 않습니다. |
| 본문 | 실제 콘텐츠 바이트입니다. 여기서 `Content-Length: 5`는 시작 줄·헤더·구분자를 제외한 5바이트를 지정합니다. 유니코드 문자 수가 아닌 바이트 수를 셉니다. |

**의미와 프레이밍은 서로 다른 질문에 답합니다.** 메서드는 요청 동작을 나타냅니다. GET은 표현 조회, HEAD는 응답 콘텐츠 없이 대응하는 응답 메타데이터 조회, POST는 대상에 전달한 콘텐츠의 처리를 요청합니다. 상태 코드의 첫 자리는 1xx 정보, 2xx 성공, 3xx 리다이렉션, 4xx 클라이언트 오류, 5xx 서버 오류로 결과를 요약합니다. `Content-Type`은 콘텐츠 해석 방법을 알려주지만 경계를 정하지 않습니다. 이러한 공통 의미는 [RFC 9110](https://www.rfc-editor.org/rfc/rfc9110.html)을 참고하세요.

HTTP/1.1 프레이밍은 재사용하는 TCP 스트림에서 어느 바이트까지가 한 메시지인지 결정합니다. 일반적인 본문 포함 메시지에서 `Transfer-Encoding`이 없다면 유효한 `Content-Length`가 길이를 지정합니다. `Transfer-Encoding: chunked`를 사용하면 청크 크기와 마지막 0 크기 청크, 그 뒤의 트레일러(있는 경우)와 최종 빈 줄로 경계를 정합니다. 송신자는 두 필드를 함께 보내면 안 됩니다. 일부 응답은 연결 종료로 경계를 정하지만 TCP 패킷 경계는 HTTP 메시지 경계가 아닙니다.

메서드·상태 규칙이 먼저 적용됩니다. HEAD 응답과 1xx·204·304 응답에는 허용된 메타데이터가 표현을 설명하더라도 메시지 본문이 없습니다. 성공한 CONNECT 응답은 터널을 시작합니다. 길이와 전송 코딩이 모두 없는 요청에도 본문이 없습니다. 이를 구분해야 다음 메시지를 콘텐츠로 잘못 읽지 않습니다([RFC 9112 §§2–6](https://www.rfc-editor.org/rfc/rfc9112.html)).

**HTTP/2와 HTTP/3도 의미는 유지하지만 바이너리 프레이밍을 사용합니다.** 텍스트 시작 줄과 CRLF 경계 대신 HEADERS·DATA 프레임 등을 사용하며, HTTP/2는 TCP 위에서, HTTP/3은 QUIC 스트림 위에서 메시지를 전달합니다. 둘 다 HTTP/1.1의 chunked 전송 코딩을 사용하지 않습니다. 도구가 디코딩한 필드를 읽기 쉬운 텍스트로 보여주어도 실제 전송 인코딩은 다를 수 있습니다([RFC 9113](https://www.rfc-editor.org/rfc/rfc9113.html), [RFC 9114](https://www.rfc-editor.org/rfc/rfc9114.html)).

HTTPS에서는 TLS가 전송 중인 HTTP 헤더와 본문을 보호하므로 세션 비밀값 없는 수동 캡처에서 이 평문을 읽을 수 없습니다([RFC 8446 §5](https://www.rfc-editor.org/rfc/rfc8446.html#section-5)). QUIC도 HTTP/3 애플리케이션 데이터를 보호합니다. 권한이 있는 끝점이나 TLS 종료 지점에서 디코딩된 메시지를 확인하고 어느 연결 구간을 관측하는지 구분하세요.

> 📎 [Linux HTTP 메시지 실습](../networking/07-linux-network-diagnostics.md#http-message-lab)을 진행한 뒤 같은 구분을 컨테이너 서비스·쿠버네티스 Ingress·AWS 로드밸런서에 적용하세요.
