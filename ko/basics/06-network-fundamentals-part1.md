...
#### IPv4와 CIDR: 서브넷 계산 예제 {#ipv4-cidr-subnet}

IPv4는 8비트 옥텟 네 개로 구성됩니다. CIDR 표기에서 `/26`은 앞의 26비트를 네트워크 프리픽스로 고정하고 `32 − 26 = 6`비트를 호스트 부분으로 남깁니다. 아래 주소들은 [RFC 5737](https://www.rfc-editor.org/rfc/rfc5737.html)의 문서용 예제이며 공개 테스트 서버 주소가 아닙니다.

인터페이스 주소가 **`192.0.2.130/26`**이라면 다음과 같이 계산합니다.

| 단계 | 계산 또는 결과 |
|---|---|
| 서브넷 마스크 | `255.255.255.192`; 마지막 옥텟의 이진수는 `11000000` |
| 블록당 주소 수 | `2^6 = 64`; 마지막 옥텟의 블록 시작점은 `0`, `64`, `128`, `192` |
| 네트워크 주소 | `130 AND 192 = 128` (`10000010 AND 11000000 = 10000000`), 따라서 `192.0.2.128/26` |
| 브로드캐스트 주소 | 호스트 6비트를 모두 1로 설정: `192.0.2.191` |
| 일반 호스트 범위 | `192.0.2.129`–`192.0.2.190`: 62개 |

인터페이스가 사용하는 주소는 `.130`이고 `.128/26`은 서브넷을 나타냅니다. 일반적인 브로드캐스트 서브넷에서는 호스트 부분이 모두 0인 주소와 모두 1인 주소를 네트워크·브로드캐스트 주소로 구분합니다. 게이트웨이도 설정한다면 호스트 범위의 주소 하나를 사용하며, CIDR 자체가 첫 호스트 주소를 게이트웨이로 강제하지는 않습니다.

모든 프리픽스에서 “주소 두 개 빼기”를 적용하면 안 됩니다. [RFC 3021](https://www.rfc-editor.org/rfc/rfc3021.html)은 지원되는 점대점 링크에서 `/31`의 두 주소를 모두 사용하도록 합니다. `/32`는 주소 하나를 가리키며 호스트 경로 등에 쓰이지만, Ethernet에 직접 연결된 이웃이라는 뜻은 아닙니다. 클라우드 할당 규칙도 별개입니다. 일반적인 AWS VPC IPv4 서브넷은 처음 네 주소와 마지막 주소를 예약하므로 **`/26`에서 할당 가능한 주소는 59개**이며 BYOIP 같은 예외도 있습니다. 할당 방식별 [VPC 서브넷 규칙](https://docs.aws.amazon.com/vpc/latest/userguide/subnet-sizing.html)을 확인하세요.

#### 최장 프리픽스와 다음 홉, ARP {#longest-prefix-next-hop}

라우팅 조회가 이웃 주소 확인보다 **먼저**입니다. `eth0`에 `192.0.2.130/26`이 있는 Linux 호스트의 예시 라우팅 테이블을 보겠습니다.

| 목적지 프리픽스 | 다음 홉 / 인터페이스 |
|---|---|
| `192.0.2.128/26` | `eth0`에 직접 연결(on-link) |
| `198.51.100.0/24` | `eth0`의 `192.0.2.129` 경유 |
| `198.51.100.128/25` | `eth0`의 `192.0.2.190` 경유 |
| `0.0.0.0/0` | 기본 경로: `eth0`의 `192.0.2.129` 경유 |

선택된 테이블 안에서 목적지와 일치하는 **가장 긴 프리픽스**를 고릅니다([RFC 1812 §5.2.4.3](https://www.rfc-editor.org/rfc/rfc1812.html#section-5.2.4.3)).

- `192.0.2.150`은 `/26` 경로로 직접 연결됩니다. 이웃 항목이 없다면 ARP로 **`.150` 자체의 MAC**을 확인합니다.
- `198.51.100.140`은 `/24`, `/25`, `/0`에 모두 해당하므로 `/25`가 선택됩니다. ARP 대상은 원격 목적지가 아니라 게이트웨이 **`192.0.2.190`**입니다.
- `203.0.113.10`은 `/0`에만 해당하므로 게이트웨이 **`192.0.2.129`**를 사용합니다.

라우팅되는 패킷의 Ethernet 목적지는 게이트웨이 MAC이지만, NAT가 없다면 IP 목적지는 원격 호스트 그대로입니다. 기본 경로의 metric이 더 낮아도 일치하는 `/25`보다 우선하지 않습니다. Linux 정책 규칙은 다른 테이블을 선택할 수 있으므로 [ip-route(8) 매뉴얼](https://man7.org/linux/man-pages/man8/ip-route.8.html)에 따라 해당 네트워크 네임스페이스에서 `ip route get`으로 실제 조회 결과를 확인하세요. 호스트와 컨테이너의 경로·이웃 테이블은 다를 수 있습니다.
...
#### ICMP와 traceroute 결과 해석 {#icmp-traceroute-interpretation}

IPv4 패킷을 전달하는 라우터는 TTL을 줄입니다. TTL이 만료되면 해당 탐색 패킷(probe)을 폐기하고 보통 **ICMP Time Exceeded(Type 11, Code 0)**를 반환합니다. 이는 의도적으로 TTL을 작게 설정한 탐색에서 기대하는 동작이며, 일반 애플리케이션 패킷도 실패한다는 증거가 아닙니다. 오류 메시지는 원본 패킷 일부를 포함하므로 송신자는 어느 probe에 대한 응답인지 연결할 수 있습니다([RFC 792](https://www.rfc-editor.org/rfc/rfc792.html)).

**내보낸 probe**와 **돌아온 응답**을 구분하세요. 아래 traceroute 방식은 모두 중간 라우터에서 ICMP Time Exceeded를 받을 수 있습니다.

| 송신 probe | 목적지에 도착했을 때의 일반적인 응답 |
|---|---|
| 사용하지 않는 목적지 포트로 UDP 전송 | ICMP Destination Unreachable의 Port Unreachable(IPv4 Type 3, Code 3) |
| ICMP Echo Request | ICMP Echo Reply |
| 선택한 포트로 TCP SYN 전송 | 수신 대기 포트는 TCP SYN/ACK, 닫힌 포트는 RST |

기본값과 옵션은 구현마다 다릅니다. [Linux traceroute(8) 매뉴얼](https://man7.org/linux/man-pages/man8/traceroute.8.html)은 UDP·ICMP·TCP 방식을 설명합니다. TCP probe는 해당 포트의 처리를 확인하지만 SYN/ACK을 받아도 TLS나 HTTP 동작까지 증명하지는 않습니다.

`*`는 **대기 시간 안에 대응하는 응답을 받지 못했다**는 뜻입니다. probe가 필터링되었거나, 라우터가 응답을 생략·속도 제한했거나, 반환 경로에서 응답이 유실되었을 수 있습니다. 한 홉이 조용해도 뒤 홉들이 응답한다면 그것만으로 종단 간 손실을 확정할 수 없습니다. 표시된 RTT에는 순방향과 다를 수 있는 반환 경로도 포함되고, 로드밸런싱 때문에 probe마다 다른 라우터가 보일 수도 있습니다. 반복 관측을 목적지·애플리케이션 결과와 대조한 뒤 장애 위치를 판단하세요.

traceroute는 주로 홉을 탐색하므로 작은 probe의 성공이 경로 MTU를 검증하지는 않습니다. DF가 설정된 더 큰 IPv4 패킷에서는 작은 MTU를 알리기 위한 **Fragmentation Needed(Type 3, Code 4)**가 여전히 필요할 수 있습니다. 이를 Time Exceeded·Port Unreachable과 구분하고 위에서 설명한 IPv6·PLPMTUD 차이도 함께 고려하세요.

> 📎 [Linux 라우팅·ICMP 실습](../networking/07-linux-network-diagnostics.md#routing-icmp-lab)에서 경로와 probe 해석을 적용하고 컨테이너·쿠버네티스 진단으로 이어가세요.
