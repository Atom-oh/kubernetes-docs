...
![클라이언트 요청이 Gateway를 거쳐 파드의 Queue Proxy로 직접 전달되거나 Scale-to-Zero 상태에서는 Activator를 거쳐 전달되고, Queue Proxy가 보고한 동시성 메트릭으로 Autoscaler가 스케일을 결정하며 Controller와 Webhook이 Knative 리소스를 관리하는 Knative Serving 아키텍처를 보여준다.](../.gitbook/assets/ko-autoscaling-03-knative-0.png)
...
![Knative Service가 Configuration과 Route를 소유하고, Configuration이 변경될 때마다 불변 Revision을 생성하며, Route가 최신 Revision에 트래픽 100%를 보내고 이전 Revision은 0%로 유지하는 리소스 모델을 보여준다.](../.gitbook/assets/ko-autoscaling-03-knative-2.png)
