...
flowchart LR
    subgraph KE[KubernetesExecutor]
        SCH1[Scheduler] -- "태스크마다 Pod 1개 생성" --> T1[Pod: task A]
        SCH1 -- "태스크마다 Pod 1개 생성" --> T2[Pod: task B]
        SCH1 -- "태스크마다 Pod 1개 생성" --> T3[Pod: task C]
    end

    subgraph CE[CeleryExecutor]
        SCH2[Scheduler] -- "태스크 큐잉" --> BROKER[(Redis broker)]
        BROKER -- "warm worker가 가져감" --> W1[Pod: celery worker 1]
        BROKER -- "warm worker가 가져감" --> W2[Pod: celery worker 2]
    end
```
