...
![사용자가 StartJobRun을 호출하면 EMR 컨트롤 플레인이 가상 클러스터에 매핑된 EKS 네임스페이스를 지정하고, 그 네임스페이스에서 작업 실행 역할(IRSA 방식)로 드라이버 Pod가 생성되며, 드라이버가 Executor Pod를 요청한 뒤 상태·로그·메트릭이 EMR로 비동기 보고되는 시퀀스를 보여준다.](../../.gitbook/assets/ko-data-on-eks-spark-03-emr-on-eks-0.png)

[🔍 인터랙티브 다이어그램 보기](https://www.atomai.click/kubernetes-docs/archmaps/ko-data-on-eks-spark-03-emr-on-eks-0.html)
