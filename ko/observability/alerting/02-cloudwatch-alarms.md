...
stateDiagram-v2
    ...
    OK --> ALARM: 임계값 초과
    ALARM --> OK: 정상 복귀
    OK --> INSUFFICIENT_DATA: 데이터 없음
    ALARM --> INSUFFICIENT_DATA: 데이터 없음
    INSUFFICIENT_DATA --> OK: 데이터 수신
    INSUFFICIENT_DATA --> ALARM: 데이터 수신 + 임계값 초과
...
graph TB
    ...
    subgraph CompositeAlarm["Composite Alarm"]
        ...
        Rule["규칙: (CPU AND Memory)<br/>OR Disk"]
    end
...
graph TB
    ...
    C --> C1[중요도별 해상도 설정]
    C --> C2[High Resolution 최소화]

    D --> D1[여러 Metric Alarm을<br/>하나의 Composite로]
