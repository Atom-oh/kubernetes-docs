...
![EKS API 서버가 하이브리드 노드에서 실행되는 웹훅 Pod를 호출할 때 컨트롤 플레인 ENI, VPC 라우트 테이블, 게이트웨이 EC2의 VXLAN 터널을 거쳐 Cilium Agent에서 디캡슐화되어 요청이 전달되고, 응답이 같은 경로를 역순으로 되돌아오는 시퀀스를 보여준다.](../.gitbook/assets/ko-eks-hybrid-nodes-10-hybrid-nodes-gateway-4.png)
