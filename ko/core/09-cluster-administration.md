...
![kube-apiserver를 중심으로 etcd, kube-scheduler, kube-controller-manager, cloud-controller-manager가 양방향으로 통신하고, 워커 노드의 kubelet이 API 서버와 양방향으로 통신하며 kube-proxy와 컨테이너 런타임을 관리하는 구조를 보여준다.](../.gitbook/assets/ko-core-09-cluster-administration-2.png)
...
![사용자 또는 서비스 계정의 요청이 인증, 권한 부여, 어드미션 컨트롤을 차례로 통과해 API 서버에 도달하며, 인증에는 X.509·토큰·OIDC·웹훅 방식이, 권한 부여에는 RBAC·ABAC·Node·Webhook 모드가 쓰인다는 것을 보여준다.](../.gitbook/assets/ko-core-09-cluster-administration-4.png)
