...
> **支持的版本**: VictoriaMetrics 1.x
> **最后更新**: February 20, 2026

## 目录

- [简介](#introduction)
- [架构选项](#architecture-options)
...
- [长期存储配置](#long-term-storage-configuration)
- [降采样](#downsampling)
...
| 功能 | 描述 |
|---------|-------------|
| **高压缩率** | 数据压缩效率最高可达 Prometheus 的 7 倍 |
| **快速查询性能** | 复杂查询性能最高可提升 20 倍 |
| **水平扩展** | 集群模式下可无限扩展 |
| **低运维开销** | 单个二进制文件部署，配置极简 |
| **兼容 Prometheus** | 完整支持 PromQL、Remote Write/Read API |
| **多租户** | 为多个团队/项目提供隔离环境 |
| **长期存储** | 高效的长期指标存储和降采样 |

### VictoriaMetrics 与 Prometheus 对比
...
| 项目 | Prometheus | VictoriaMetrics |
|------|------------|-----------------|
| 架构 | 单节点 | 单节点/集群 |
| 水平扩展 | 需要 Thanos/Cortex | 原生支持 |
| 磁盘用量 | 基准 | 减少约 70% |
| 查询速度 | 基准 | 快 2-20 倍 |
| 内存用量 | 高 | 低 |
...
| 组件 | 角色 | 扩展方式 |
|-----------|------|----------------|
| **vminsert** | 写入请求路由 | 水平扩展（Deployment） |
| **vmstorage** | 数据存储 | 水平扩展（StatefulSet） |
| **vmselect** | 查询处理 | 水平扩展（Deployment） |
...
vmagent 是用于指标收集和转发的轻量级代理。

### 主要功能

- 兼容 Prometheus 抓取配置
- 支持多个 Remote Write 目标
- 数据缓冲和重传
- 资源用量低
- 标签重写和筛选
...
MetricsQL 是 VictoriaMetrics 的查询语言，也是 PromQL 的超集。

### PromQL 兼容性

所有 PromQL 查询都可在 MetricsQL 中运行：
...
2. **高可用性**
   - 在集群模式下设置 `replicationFactor=2`
   - 至少使用 2 个 vmselect 和 vminsert 副本
   - 至少使用 3 个 vmstorage 副本
...
若要测试您对本章内容的理解，请尝试 [VictoriaMetrics 测验](../../quizzes/observability/metrics/02-victoriametrics-quiz.md)。
