...
本测验用于测试你对 Kubernetes 环境中使用的 Linux 操作技能的理解。
...
7. `ssh -L 8080:localhost:80 user@server` 表示什么？
   - A) 将服务器的 8080 转发到本地的 80
   - B) 将本地的 8080 转发到服务器的 80
   - C) 将服务器的 80 转发到本地的 8080
   - D) 将本地的 80 转发到服务器的 8080

<details>
<summary>显示答案</summary>

**答案：B) 将本地的 8080 转发到服务器的 80**
...
16. 编写一个脚本，要求必须提供 DATABASE_URL，并将 TIMEOUT 默认值设为 30。
...
17. 编写一个命令，以 JSON 形式输出重启次数为 3 次及以上的 Pods。
...
18. 编写一个通过 bastion 同步 yaml 文件的 rsync 命令。
