...
A. 总时限 10 秒内最多 Retry 3 次，每次尝试限制为 2 秒 B. 总时限 2 秒内最多 Retry 3 次，每次尝试限制为 10 秒 C. 总时限 10 秒内无限 Retry，每次尝试限制为 2 秒 D. 10 秒后失败且不 Retry
...
此配置在**总计 10 秒**内**最多 Retry 3 次**，**每次尝试限制为 2 秒**。
...
* `timeout` >= `attempts x perTryTimeout`，以允许所有 Retry
* 过多的 Retry 可能导致级联故障
* 仅建议对幂等操作使用 Retry
...
说明如何结合使用 Argo Rollouts 和 Istio 实现自动化 Canary Deployment。请包括**所需资源**（Rollout、VirtualService、DestinationRule、AnalysisTemplate）和**自动回滚条件**。
...
**重要提示**: Rollout 会自动向 Pod 添加 `rollouts-pod-template-hash` label，并使用此 label 区分子集。
...
1. **完全自动化**: Deployment 无需人工干预即可推进
2. **即时回滚**: 检测到指标失败后数秒内回滚
3. **安全 Deployment**: 每个阶段都自动验证
...
比较 Blue/Green Deployment 和 Canary Deployment 的**差异**，并说明各自的**优缺点**和**适用场景**。
...
4. **自动化 Deployment**: CI/CD 中的自动 Deployment
5. **Microservices**: 服务依赖关系复杂时
...
说明如何使用流量镜像安全地测试新版本。请包括**用例**、**配置方法**和**注意事项**。
...
说明如何使用 Istio 的 Locality Load Balancing 在 AWS EKS 中**降低跨 AZ 成本**。请包括配置示例和**预计成本节省**。
...
Locality Load Balancing 是一项优先路由到**同一 Availability Zone (AZ)** 中 Service 的功能，用于降低网络延迟和跨 AZ 成本。
...
* [流量管理文档](../../../service-mesh/istio/traffic-management/)
