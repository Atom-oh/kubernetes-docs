...
eBPF 是一项革命性技术，允许沙箱化程序在 Linux kernel 内运行。本文档涵盖从 eBPF 基本概念到 Kubernetes 环境中实际应用的全部内容。
...
* [8. 实用 eBPF 示例](#8-practical-ebpf-examples)
...
**eBPF (extended Berkeley Packet Filter)** 是一种允许用户定义的程序安全地在 Linux kernel 内运行的技术。它最初作为 BPF 为网络数据包过滤而设计，后来经过扩展，现在用于网络、安全、跟踪和性能分析等多个领域。

> **核心概念**: eBPF 允许你在不修改 kernel 源代码或加载 kernel modules 的情况下扩展和观测 kernel 行为。
...
| 指令数量 | 4,096 | 1 million+ |
| Map 支持 | 无 | 多种 map 类型 |
| 使用场景 | 数据包过滤 | 通用 kernel 编程 |
| 调用能力 | 无 | Helper functions, BPF-to-BPF calls |
...
1. **无需修改 kernel 即可扩展功能**: 在不更改 kernel 源代码的情况下扩展 kernel 功能
2. **安全执行**: Verifier 保证程序安全性
3. **高性能**: 通过 JIT 编译获得接近原生代码级别的性能
4. **动态加载**: 无需重启即可加载/卸载程序
5. **生产稳定性**: 安全执行，不会崩溃或陷入无限循环
...
| **权限** | CAP_BPF 或 CAP_SYS_ADMIN | 需要 Root privileges |
| **调试** | 有限 | 可进行完整 kernel 调试 |
| **性能** | 通过 JIT 编译优化 | 原生性能 |
| **功能范围** | 仅指定 hook points | 无限制 |
| **开发难度** | 相对容易 | 需要高专业性 |
...
JIT (Just-In-Time) compiler 将 eBPF bytecode 转换为原生 machine code：
...
eBPF 程序通过 kernel 提供的 helper functions 访问 kernel functions。
...
| 功能 | XDP | TC |
|---------|-----|-----|
| 执行位置 | Driver level | Network stack |
| 性能 | 最高 | 高 |
| SKB 访问 | 不可能 | 可能 |
| 方向 | 仅 Ingress | 同时支持 ingress 和 egress |
| 数据包修改 | 有限 | 灵活 |

### 3.3 Kprobes/Uprobes

Kprobes 和 Uprobes 可动态跟踪函数调用。
...
Tracepoints 是 kernel 中预定义的静态跟踪点。
...
BCC 是一个工具包，允许通过 Python 和 Lua 编写 eBPF 程序。
...
libbpf 是用于加载 eBPF 程序的 C library，并支持 CO-RE (Compile Once, Run Everywhere)。
...
| 方面 | iptables | eBPF |
|--------|----------|------|
| **可扩展性** | O(n) - 与 service 数量成比例 | O(1) - map lookup |
| **延迟** | 随规则数量增加 | 恒定 |
| **CPU 使用率** | 高 | 低 |
| **更新** | 重写整张表 | 更新 map entry |
| **可观测性** | 有限 | Hubble integration |
| **内存** | 每条规则占用内存 | 优化的 map 结构 |

**基准测试结果**（基于 1000 个 services）：
...
eBPF 能够深入观测系统和应用行为。与传统基于 agent 的监控不同，eBPF 在 kernel 级别收集数据，以更低开销提供更丰富的信息。
...
Pixie 使用 eBPF 自动收集 telemetry，无需修改应用代码。
...
| 方面 | Traditional Agents | eBPF Instrumentation |
|--------|-------------------|---------------------|
| **开销** | 高 (5-15%) | 低 (<1%) |
| **代码修改** | 需要 (SDK/library) | 不需要 |
| **覆盖范围** | 仅已 instrumented 的部分 | 整个系统 |
| **部署** | 每个应用 | 每个 node |
| **权限** | 普通权限 | 需要 CAP_BPF |
| **数据深度** | 应用级别 | Kernel 级别 |
| **协议支持** | 需要显式支持 | 自动解析 |
...
Tetragon 是 Cilium project 提供的基于 eBPF 的 runtime security 解决方案。
...
Falco 是一个 CNCF project，使用 eBPF 检测 runtime 异常行为。
...
LSM BPF 将 Linux Security Module 与 eBPF 结合，用于动态应用安全策略。
...
### 8.2 使用 Cilium Hubble 观测 Network Flow
...
| `CAP_PERFMON` | 附加到 performance monitoring events |
...
- [BPF Performance Tools](https://www.brendangregg.com/bpf-performance-tools-book.html) - Brendan Gregg 的 BPF performance tools 书籍

**实践环境**:
- [eBPF Tutorial](https://github.com/lizrice/learning-ebpf) - Liz Rice 的 eBPF tutorial
- [BCC Tutorial](https://github.com/iovisor/bcc/blob/master/docs/tutorial.md) - 官方 BCC tutorial
- [bpftrace Tutorial](https://github.com/iovisor/bpftrace/blob/master/docs/tutorial_one_liners.md) - bpftrace one-liner tutorial

**社区**:
- [eBPF Summit](https://ebpf.io/summit/) - 年度 eBPF conference
- [Cilium Slack](https://cilium.io/slack) - Cilium community

### 10.3 相关文档

有关与本文档相关的高级内容，请参阅以下内容：

| 主题 | 文档链接 | 描述 |
|-------|---------------|-------------|
| Cilium 简介 | [Cilium Overview](../networking/cilium/01-introduction.md) | 基于 eBPF 的 CNI 介绍 |
| eBPF 深入解析 | [eBPF Technical Deep Dive](../networking/cilium/02-ebpf.md) | 高级 eBPF 技术 |
...
| Kubernetes 网络 | [Services and Networking](../core/03-services-networking.md) | 基本网络概念 |
...
eBPF 是一项革命性技术，允许安全地扩展和观测 Linux kernel 行为。以下是本文档涵盖的关键内容摘要：

1. **eBPF 基本概念**: 在 kernel 内安全运行的沙箱化程序
...
6. **可观测性**: 通过 Hubble, Pixie, Coroot 进行深度系统观测
7. **安全**: 通过 Tetragon, Falco, seccomp-bpf 实现 runtime security
8. **限制**: 需要考虑 stack size、loops、kernel 版本兼容性

eBPF 是引领 cloud-native 环境中网络、安全与可观测性未来的核心技术。
