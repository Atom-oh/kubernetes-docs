...
> **支持版本**: All major Linux distributions (Ubuntu 20.04+, CentOS/RHEL 8+, Debian 11+) **最后更新**: February 11, 2026

理解 Linux 基础知识对于掌握 Kubernetes 和容器技术至关重要。本文档介绍在 Kubernetes 环境中特别重要的核心 Linux 概念。

## Lab Environment Setup

要跟随本文档中的示例进行练习，你需要以下环境：
...
Linux 内核是操作系统的核心，充当硬件和软件之间的中介。它的主要功能包括：

* **进程管理**：进程创建、调度和终止
* **内存管理**：虚拟内存和物理内存分配
* **设备管理**：与硬件设备通信
* **系统调用接口**：为用户空间程序访问内核服务提供方式

### User Space

用户空间是普通应用程序运行的内存区域。用户空间程序通过系统调用访问内核服务。
...
* **线程**：在进程内部执行的工作单元；同一进程的线程共享内存空间

### Process States

* **运行中**：当前正在 CPU 上执行
* **等待中**：等待 I/O 完成或事件发生
* **就绪**：已准备好运行，但正在等待 CPU 分配
* **僵尸**：已终止，但父进程尚未检查其状态
* **已停止**：挂起状态
...
Namespaces 是 Linux 内核的一项功能，用于隔离进程组，使每个组都能独立查看系统资源。这是容器技术的核心要素。
...
* **IPC Namespace**：进程间通信资源隔离（共享内存、信号量、消息队列等），对于微服务架构中的服务隔离很重要
* **User Namespace**：用户和组 ID 隔离，支持 rootless 容器执行以增强安全性
* **cgroup Namespace**：cgroup 根目录隔离，在容器内部提供资源限制可见性
...
* **Freezer**：暂停和恢复进程组（用于容器暂停）
* **cpuset**：将进程绑定到特定 CPU 核心和 NUMA 节点

### cgroups v1 and v2

* **cgroups v1**：每种资源类型使用独立层级，仍用于旧系统
* **cgroups v2**：统一的单一层级，用于更一致的管理，是现代发行版的默认选择
...
Linux 文件权限由所有者、组和其他用户的读（r）、写（w）和执行（x）权限组成。
...
* **Timers**：可替代 cron 的 timer units
* **资源管理**：通过 cgroups 设置按服务划分的资源限制

### systemd Unit Types

* **service**：系统服务（例如 kubelet.service、containerd.service）
* **socket**：基于 socket 的激活
* **target**：Unit 组（类似 runlevels）
* **timer**：计划任务
* **mount**：文件系统挂载
* **device**：设备 Unit
...
Kubernetes 相关服务的 systemd unit 文件示例：
...
ulimit 限制进程可以使用的系统资源。为了确保资源充足，可能需要在 Kubernetes 节点上进行调整。
...
chronyd 是一种现代 NTP 客户端，比 ntpd 同步时间更快。
...
用于安装和管理 Kubernetes 及相关工具的包管理器用法。
...
OverlayFS 是一种 union mount 文件系统，可将多个目录呈现为单个目录。Docker 等容器运行时使用它来实现镜像层。

### Network Bridge and NAT

容器网络主要使用桥接接口和 NAT（Network Address Translation）实现。
...
Linux capabilities 将传统 root 权限拆分为更小的权限单元。容器只接收必要的 capabilities，以增强安全性。

关键 capabilities：

* `CAP_NET_ADMIN`：网络配置变更
...
Linux 基础知识和功能对于理解 Kubernetes 和容器技术至关重要。以下是本文档涵盖的关键主题摘要：
...
* **网络**：DNS、bridge、iptables 配置
* **包管理**：Kubernetes 组件的版本管理

借助这些 Linux 基础知识，你可以在 Kubernetes 环境中有效排查问题、优化集群并可靠地运行集群。

## Quiz

要测试你在本章中学到的内容，请完成 [Linux 基础测验](../quizzes/basics/01-linux-basics-quiz.md)。
