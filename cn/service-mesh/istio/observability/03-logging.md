...
Istio 的日志功能可让您记录并分析服务网格中的所有活动。使用 Access Log（访问日志）、Envoy 日志和 structured logs（结构化日志）进行流量分析、调试和安全审计。

## 目录

1. [日志概览](#logging-overview)
2. [Access Log 配置](#access-log-configuration)
3. [使用 Telemetry API 自定义日志](#log-customization-with-telemetry-api)
4. [日志过滤与采样](#log-filtering-and-sampling)
5. [调整 Envoy 日志级别](#envoy-log-level-adjustment)
6. [Promtail + Loki 集成](#promtail--loki-integration)
...
1. **Access Log（访问日志）**：记录所有 HTTP/TCP 请求/响应
2. **Envoy Proxy 日志**：Envoy 内部操作日志
3. **Istiod 日志**：控制平面日志
4. **应用日志**：应用自身的日志

## Access Log 配置

### 1. 使用 MeshConfig 启用全局 Access Log
...
### 2. 使用 Telemetry API 进行精细控制

Telemetry API 允许您按 namespace 或 workload 控制日志。

#### 为整个网格启用 JSON Access Log
...
| `request.url_path` | URL 路径（不含查询参数） | /api/v1/users |
| `request.headers` | 请求头 | `request.headers['user-agent']` |
| `response.code` | HTTP 状态码 | 200, 404, 500 |
| `response.headers` | 响应头 | `response.headers['content-type']` |
| `response.flags` | Envoy 响应标志 | UH, UF, URX |
| `duration` | 请求时长（ms） | 123 |
| `connection.mtls` | mTLS 使用情况 | true, false |
| `source.principal` | 源服务账户 | spiffe://... |
| `destination.principal` | 目标服务账户 | spiffe://... |
...
| `jwt` | JWT 身份验证 | JWT token 验证 |
| `lua` | Lua 脚本 | Lua 过滤器调试 |
| `main` | 主逻辑 | 一般 Envoy 操作 |
| `router` | 路由 | 路由决策跟踪 |
| `runtime` | 运行时配置 | 动态配置变更 |
| `upstream` | 上游 cluster | 后端连接问题 |
...
### 2. 安装 Promtail（Istio Access Log 收集）
...
在 Grafana 中，您可以直接从 Loki 日志跳转，以查看特定请求的追踪。
...
**使用方法**：
1. 在 Grafana Explore 中查看 Loki 日志
2. 单击日志条目中的 “View Trace” 链接
3. 自动跳转到 Tempo 以查看相应的追踪
