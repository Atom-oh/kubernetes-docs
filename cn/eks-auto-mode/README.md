...
Amazon EKS Auto Mode 是一项完全自动化 Kubernetes 节点管理的功能，可根据 workload（工作负载）需求自动预置和优化节点。本指南涵盖 EKS Auto Mode 的概念、配置方法以及生产环境的最佳实践。

## 目录

1. [Auto Mode 入门](./01-getting-started.md) - Cluster 创建和启用 Auto Mode
2. [NodePool 配置和优化](./02-nodepool-configuration.md) - 默认和自定义 NodePools
3. [理解扩展行为](./03-scaling-behavior.md) - 预置、整合、漂移检测
4. [Spot Instance 利用策略](./04-spot-strategies.md) - 混合容量和中断处理
5. [操作和管理](./05-operations.md) - 中断预算、滚动替换、监控
6. [成本管理和优化](./06-cost-management.md) - 成本分析、Spot 节省、right-sizing
7. [Node 生命周期管理](./07-node-lifecycle.md) - 过期、AMI 管理、freshness 策略
8. [Workload 特定优化](./08-workload-optimization.md) - Web、batch、GPU、AI/ML workloads
...
EKS Auto Mode 是由 AWS 管理的完全自动化节点管理解决方案。它内部基于 Karpenter，AWS 会管理所有内容，用户无需安装或配置单独的节点管理组件。
...
|---------|---------------------|---------|-----------|
| Node 管理 | 用户管理（基于 ASG） | 完全由 AWS 托管 | 完全由 AWS 托管 |
| 扩展方法 | Cluster Autoscaler | 按 Pod | 基于 Karpenter |
| 扩展速度 | 数分钟 | 即时（Pod 调度） | 数十秒 |
| Instance Type 选择 | 预定义 | 自动 | 自动优化 |
| Spot 支持 | 手动配置 | 不支持 | 自动管理 |
| GPU Workloads | 支持 | 有限 | 完全支持 |
...
EKS Auto Mode 基于 Karpenter 运行，但在 AWS 托管的 control plane（控制平面）内运行。
...
| 每个 cluster 的最大节点数 | 5000 |
| 最低 EKS 版本 | 1.29 |
| 支持的 AMI families | AL2023, Bottlerocket |
| Windows 节点 | 不支持 |
...
要检验你的学习成果，请尝试 [EKS Auto Mode 测验](../quizzes/eks-auto-mode/01-getting-started-quiz.md)。
...
- [从 self-managed Karpenter 迁移到 EKS Auto Mode](https://docs.aws.amazon.com/eks/latest/userguide/auto-migrate-karpenter.html)

---

< [返回 EKS 主题](../README.md) | [下一篇：入门](./01-getting-started.md) >
