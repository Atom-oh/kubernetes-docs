...
- 部署可观测性的三大支柱：Metrics、Logs 和 Traces
- 将 OpenTelemetry Collector 配置为中央遥测管道
- 实现多后端扇出，以提供冗余和灵活性
- 设置集成统一数据源的 Grafana
- 使用 Alertmanager 和 SNS 配置基础告警

## 前提条件

- [ ] 已完成[第 1 部分：基础设施设置](./01-infrastructure-setup-lab.md)
- [ ] Managed Cluster 和 Service Cluster 正在运行
- [ ] AWS 托管服务（AMP、OpenSearch）可用
- [ ] 已将 ArgoCD 配置为支持多 Cluster 部署
...
## 练习 1：OpenTelemetry Collector 部署

### 步骤

**步骤 1.1：切换到 Managed Cluster**
...
**步骤 1.2：创建 OpenTelemetry namespace 和 ConfigMap**
...
**步骤 1.3：部署 OTel Collector DaemonSet（Agent）**
...
**步骤 1.4：部署 OTel Collector Gateway（Deployment）**
...
**步骤 2.1：安装 kube-prometheus-stack**
...
**步骤 2.2：安装 VictoriaMetrics（备用 Metrics 后端）**
...
**步骤 3.2：安装 ClickHouse 以进行高性能日志分析**
...
**步骤 4.2：通过 OTel 配置 X-Ray 集成**

练习 1 中配置的 OTel Collector 已将 traces 导出至 X-Ray。请验证该配置：
...
| 组件 | 工具 | 用途 |
|-----------|------|---------|
| **遥测管道** | OTel Collector | 集中采集和扇出 |
| **Metrics** | Prometheus、VictoriaMetrics、Mimir | 时序存储 |
| **Metrics（AWS）** | AMP | 托管 Prometheus |
| **Logging** | Loki、ClickHouse、Fluent Bit | 日志聚合 |
| **Logging（AWS）** | CloudWatch Logs、OpenSearch | 托管日志 |
| **Tracing** | Tempo | 分布式追踪 |
| **Tracing（AWS）** | X-Ray | 托管追踪 |
| **可视化** | Grafana | 统一仪表板 |
| **告警** | Alertmanager、Grafana OnCall | 告警路由 |

## 清理

清理将在[第 6 部分](./06-distributed-tracing-lab.md#cleanup)中进行。

## 故障排除

<details>
<summary>OTel Collector 未接收数据</summary>

- 检查 Collector 日志：`kubectl logs -n opentelemetry -l app=otel-collector`
- 验证端口已暴露：`kubectl get svc -n opentelemetry`
- 测试 OTLP endpoint：`curl -v http://localhost:4318/v1/traces`
</details>

<details>
<summary>Loki 未存储 Logs</summary>

- 检查 S3 bucket 权限
- 验证 Fluent Bit 输出：`kubectl logs -n logging -l app.kubernetes.io/name=fluent-bit`
- 测试 Loki push：`curl -X POST http://localhost:3100/loki/api/v1/push ...`
</details>

<details>
<summary>Grafana 数据源无法连接</summary>

- 验证 Service DNS：`kubectl run -it --rm debug --image=busybox -- nslookup loki-gateway.logging.svc.cluster.local`
- 检查 Grafana UI 中的数据源配置
- 测试直接连接：`kubectl port-forward svc/loki-gateway 3100:80`
</details>

## 后续步骤

继续学习[第 3 部分：MSA 部署和 Canary](./03-msa-deployment-lab.md)，以部署带有可观测性插桩的示例应用程序。

## 参考资料

- [OpenTelemetry Collector 文档](../../observability/tracing/03-opentelemetry.md)
