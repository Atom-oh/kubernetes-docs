...
## 学习目标
- 练习 Linux 进程管理命令
- 直接观察 Linux 命名空间的隔离效果
- 通过 cgroups 了解资源限制
- 练习文件权限和所有权管理

## 先决条件
- [ ] Linux 终端访问（建议使用 Ubuntu 20.04+）
- [ ] sudo 权限
...
- K8s `resources.limits.memory` 被转换为容器的 cgroup 内存限制
- 超过限制会导致 OOMKilled 状态
- 您可以使用 `kubectl describe pod` 检查资源限制
</details>
...
<details>
<summary>找不到 unshare 命令</summary>
...
<details>
<summary>ip netns 命令不工作</summary>
