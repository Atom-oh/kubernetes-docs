...
- `@tsv` 以制表符分隔格式输出
- 在真实的 K8s 中，可以像 `kubectl get pods -o json | jq '...'` 这样使用
</details>
...
- `$((RANDOM % N))` 会生成一个从 0 到 N-1 的随机数
- `grep -oP` 使用 Perl 正则表达式仅提取匹配的部分
- `sort | uniq -c | sort -rn` 是进行频率计数的基本模式
</details>
