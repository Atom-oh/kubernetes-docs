...
- `llms.txt` — a standard llms.txt index: `# title` / `>` summary / `## Docs (한국어)` / `## Docs (English)` / `## Section bundles (…)` / `## Optional`. Each entry provides the group and title, a raw Markdown URL, and a short summary extracted from the first body paragraph.
