...
VictoriaMetrics is a high-performance, cost-effective time series database and monitoring solution. While fully compatible with Prometheus, it offers better compression rates, query performance, and scalability.

### Key Features

| Feature | Description |
|---------|-------------|
| **High Compression** | Up to 7x more efficient data compression than Prometheus |
| **Fast Query Performance** | Up to 20x faster performance on complex queries |
| **Horizontal Scaling** | Unlimited scaling in cluster mode |
| **Low Operational Overhead** | Single binary deployment, minimal configuration |
| **Prometheus Compatible** | Full support for PromQL, Remote Write/Read API |
| **Multi-tenancy** | Isolated environments for multiple teams/projects |
| **Long-term Storage** | Efficient long-term metric storage and downsampling |
...
|------|------------|-----------------|
| Architecture | Single node | Single/Cluster |
| Horizontal scaling | Requires Thanos/Cortex | Native support |
| Disk usage | Baseline | ~70% reduction |
| Query speed | Baseline | 2-20x faster |
| Memory usage | High | Low |
| Cardinality limit | ~10M time series | ~100M+ time series |
...
Single-node mode (vmsingle) is suitable for small to medium environments.

### Features

- Single binary provides all functionality
- Simple setup and operation
- Capable of processing millions of samples per second
- Supports billions of active time series
...
Distributes scrape targets across vmagent instances in large-scale environments:
...
MetricsQL is VictoriaMetrics' query language and a superset of PromQL.

### PromQL Compatibility

All PromQL queries work in MetricsQL:
...
VictoriaMetrics Enterprise supports downsampling. For open source, use recording rules.
...
2. **High Availability**
   - Set `replicationFactor=2` in cluster mode
   - Minimum 2 vmselect, vminsert replicas
   - Minimum 3 vmstorage replicas
...
To test your understanding of this chapter, try the [VictoriaMetrics Quiz](../../quizzes/observability/metrics/02-victoriametrics-quiz.md).
