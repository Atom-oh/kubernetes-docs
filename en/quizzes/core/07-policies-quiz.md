...
9. A namespace has quotas for CPU/memory requests and limits and no admission defaults. What happens to a Pod whose containers specify none of these resources?
...
**Explanation:**
When ResourceQuota is applied to a namespace and quotas are set for computing resources like CPU and memory, Pods must have the resource fields needed by that quota after defaulting. If the required values remain missing after admission defaulting, creation is rejected. A LimitRange can fill defaults; an object-count-only quota does not require CPU/memory fields. This is to accurately track and limit resource usage in namespaces with quotas applied.
</details>
...
2. **Baseline**:
   ...
   - HostPath volumes are forbidden by Baseline too; Restricted adds requirements such as non-root execution, seccomp, and capability restrictions.
...
These policies isolate the selected directions only. Other NetworkPolicies add their allowed traffic; they cannot narrow a broader allow policy. Add DNS egress separately when names are used, and test both source and destination rules.
</details>
...
Install each matching ConstraintTemplate from the official Gatekeeper library first. These kinds are generated by templates, not built-in APIs; the library K8sRequiredLabels schema differs from the chapter's simplified K8sRequiredLabelKeys.
...
4. **Restrict host filesystem access** (requires the library's matching template):
   ```yaml
   apiVersion: constraints.gatekeeper.sh/v1beta1
...
5. **Require labels with constrained values**:
   ```yaml
   apiVersion: constraints.gatekeeper.sh/v1beta1
...
3. **Leverage Preemption**:
   ...
   - This improves scheduling preference, but cannot guarantee placement if constraints or available capacity still prevent it
...
4. **Consider System Priority Classes**:
   ...
   - User-defined PriorityClass values must be at most 1,000,000,000; higher values are reserved for system classes
...
1. **Architecture and Approach**:
   ...
   - **Kyverno**: Uses its own policy engine, providing YAML/JSON-based policy definitions. Provides multiple policy kinds, including Policy/ClusterPolicy and dedicated cleanup/CEL policy APIs depending on version.
...
4. **Pros**:
   - **OPA Gatekeeper**:
     - Established Rego policy ecosystem and reusable templates
...
5. **Cons**:
   ...
   - **Kyverno**:
     - Requires version-specific CRD and feature compatibility checks
     - May have limitations for very complex policy expression
     - Benchmark representative policies and admission load; there is no universal engine performance ranking
...
6. **Selection Criteria**:
   ...
   - **Choose Kyverno when**:
     ...
     - Kubernetes-native mutation, generation, verification, or validation fits the policy requirements
</details>
...
1. **Implementation**:
   - **PodSecurityPolicy**: Implemented as built-in API resource (not a CRD), applied through admission controller.
...
3. Explain how an external authorization webhook can supplement Kubernetes RBAC, and why ordering matters.
...
For a self-managed API server, configure a TLS-authenticated webhook using a kubeconfig that supplies its endpoint, CA, client certificate, and key. A flag-based example is:
...
The webhook receives an `authorization.k8s.io/v1` SubjectAccessReview containing the authenticated user/groups and request attributes, and returns a SubjectAccessReview status such as `allowed: true` or an explicit denial. It must validate transport credentials, handle errors/timeouts deliberately, and avoid logging bearer tokens or confidential request data.

**The authorizer chain stops at the first allow or deny.** In `Node,RBAC,Webhook`, an RBAC allow skips the webhook. A later webhook cannot veto that permission. Mandatory external denial policies need an appropriate ordering/failure policy or suitable admission enforcement. Structured AuthorizationConfiguration supports additional webhook controls. EKS does not expose arbitrary API-server authorization flags.

An OPA adapter can evaluate the review with Rego. This OPA 1.x example uses current rule syntax:
...
Deploy a compatible OPA version, load the policy bundle, and implement an adapter that submits the review as OPA input and maps the result to SubjectAccessReview status. A bare OPA Deployment is not a complete authorization webhook. Centralized policy, external attributes, and auditability are benefits; availability, latency, cache TTLs, and denial behavior must be tested.

</details>

4. Design multi-tenant network isolation using NetworkPolicy.
...
Start with a default-deny ingress/egress policy in each tenant namespace. Then choose either broad intra-tenant communication or fine-grained role-based rules. Because policies are additive, a broad allow-same-namespace policy defeats narrower role restrictions if both are applied.

For the broad intra-tenant option, allow both directions; ingress alone does not undo the default egress deny:
...
For finer isolation, select labels such as `role: backend` or `tier: web` instead. Expose only selected APIs to other tenants by combining a namespace selector and Pod selector in the same peer, with explicit destination ports. Use the built-in `kubernetes.io/metadata.name` namespace label or maintain a protected tenant label convention.

Shared-service access needs matching source egress and destination ingress. Allowing tenants to reach Prometheus on 9090 exposes its API/UI; it does not authorize Prometheus to scrape tenant Pods. Scraping needs the reverse traffic path. Restrict external egress with appropriate IP ranges and TCP ports, or an implementation supporting domain policies when IPs are unsuitable.

DNS egress commonly needs the following rule (adjust for NodeLocal DNS or different labels):
...
Use a policy-enforcing CNI, protect namespace/Pod label changes with RBAC and admission rules, version policies, and test allowed and denied flows. Standard NetworkPolicy does not provide L7 authentication and has documented node/host-network limitations.

</details>

5. Explain a resource-management strategy combining ResourceQuota and LimitRange.
...
- At cluster level, plan node capacity, autoscaling, and workload isolation. At namespace level, constrain requested/limited compute, storage, and object counts. At workload level, set measured resource requests and Pod priority.
- Use `requests.storage` and `<class>.storageclass.storage.k8s.io/requests.storage` for PVC allocation. For arbitrary object counts use keys such as `count/deployments.apps` and `count/statefulsets.apps`; bare `deployments.apps` is not the object-count syntax.
- Apply a separate PriorityClass-scoped quota with `scopeSelector` and `hard.pods` for each class. PriorityClass belongs to Pods, not directly to namespaces.
- Use Container LimitRange entries for defaults and min/max validation. Pod entries constrain aggregate resources, and PVC entries constrain storage requests; neither supplies defaults.
...
ResourceQuota and LimitRange have no built-in audit-only mode: evaluate changes in test namespaces and through dry-run where applicable. Monitor quota allocation separately from actual usage, alert before limits, and adjust requests based on measurements. Dynamic quota changes, borrowing, shared pools, and chargeback need additional controllers/automation; native hard quotas do not permit temporary overages. Quotas limit admission rather than guarantee workload performance.
