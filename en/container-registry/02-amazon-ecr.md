...
![Flowchart showing how an ECR lifecycle policy evaluates a pushed image against rules in rulePriority order, applies the first matching rule, then expires the image only if it exceeds that rule's retention threshold and otherwise retains it.](../.gitbook/assets/en-container-registry-02-amazon-ecr-1.png)
