...
![Diagram showing that AI/ML workloads are resource intensive and diverse: resource intensity leads to GPU acceleration, large memory, high-performance CPU and high-speed network requirements, while workload diversity spans training, inference, preprocessing and HPO.](../.gitbook/assets/en-ai-ml-01-ai-ml-workloads-0.png)
...
![Amazon EKS cluster whose training, inference, and CPU node groups share one storage layer (EBS, EFS, FSx for Lustre, S3) and one networking layer (VPC CNI, ENA/EFA, placement groups), while the cluster integrates with SageMaker, ECR, and CloudWatch.](../.gitbook/assets/en-ai-ml-01-ai-ml-workloads-1.png)
...
![Diagram of the AI/ML software components on Amazon EKS: the NVIDIA GPU Operator deploys the node-level GPU stack (driver, Container Toolkit, Device Plugin, DCGM Exporter), Kubeflow provides notebooks, training jobs, Pipelines and Katib, KServe, TorchServe and Triton are the model serving options, and the MPI Operator runs distributed training through MPIJob.](../.gitbook/assets/en-ai-ml-01-ai-ml-workloads-2.png)
...
![Diagram showing a launcher pod starting four worker pods that exchange gradients over NVIDIA NCCL, which runs over MPI and the Elastic Fabric Adapter, while the workers write checkpoints to FSx for Lustre, which feeds Amazon S3 and a checkpoint store.](../.gitbook/assets/en-ai-ml-01-ai-ml-workloads-3.png)

[🔍 View interactive diagram](https://www.atomai.click/kubernetes-docs/archmaps/en-ai-ml-01-ai-ml-workloads-3.html)
...
![Diagram showing client requests passing through a networking layer to the inference services, which load models from model storage while scaling options automatically scale them.](../.gitbook/assets/en-ai-ml-01-ai-ml-workloads-4.png)

[🔍 View interactive diagram](https://www.atomai.click/kubernetes-docs/archmaps/en-ai-ml-01-ai-ml-workloads-4.html)
...
![Diagram showing GPU, training, and storage optimization feeding into a performance-improvement outcome while cost optimization feeds into a cost-reduction outcome, across the four AI/ML workload optimization areas.](../.gitbook/assets/en-ai-ml-01-ai-ml-workloads-5.png)
