...
For eligible meshed TCP peers, Linkerd provides transport mTLS. Unmeshed peers and explicit capture bypasses need separate consideration. The default inbound policy accepts unmeshed plaintext; automatic mTLS is not equivalent to enforcing authenticated access from every source.
...
Default inject mode is an annotation transform. Manual mode materializes the proxy and uses CLI options for overrides; it does not apply proxy configuration annotations like server-side injection. Review the result before deployment rather than copying a shortened proxy container as a complete installation.
