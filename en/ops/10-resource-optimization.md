...
- [Resource Configuration Fundamentals](#resource-configuration-fundamentals)
- [Optimal Resource Calculation](#optimal-resource-calculation)
- [JVM Workload Optimization](#jvm-workload-optimization)
- [Python/Node.js Workloads](#pythonnodejs-workloads)
- [Go/Rust Workloads](#gorust-workloads)
- [Resource Monitoring Dashboards](#resource-monitoring-dashboards)
- [Resource Optimization in Auto Mode](#resource-optimization-in-auto-mode)

---

## Resource Configuration Fundamentals

### Requests vs Limits

Understanding the difference between requests and limits is critical for proper resource management in Kubernetes.

| Aspect | Requests | Limits |
|--------|----------|--------|
| **Purpose** | Scheduling guarantee | Maximum allowed |
| **Scheduler** | Used for node placement | Not used |
| **Enforcement** | Soft (reserved) | Hard (enforced by cgroups) |
| **Overcommit** | Can exceed request | Cannot exceed limit |
| **OOM Kill** | Not triggered | Triggers OOM when exceeded |
| **Throttling** | Not applied | CPU throttled at limit |
...
| QoS Class | Criteria | OOM Priority | Use Case |
|-----------|----------|--------------|----------|
| **Guaranteed** | requests = limits for all containers | Lowest (last to be killed) | Critical workloads |
| **Burstable** | requests < limits or partial config | Medium | Most applications |
| **BestEffort** | No requests or limits | Highest (first to be killed) | Batch jobs, dev pods |
...
### CPU Throttling and CFS Bandwidth

Linux uses the Completely Fair Scheduler (CFS) for CPU management:
...
- CPU throttling causes **latency spikes**, not throughput reduction
- Applications may appear slow even when average CPU is low
- GC-intensive applications are particularly affected
- Set limits 20-50% above observed peak to avoid throttling
...
### Memory OOMKill Behavior

When a container exceeds its memory limit:

1. Kernel sends SIGKILL (cannot be caught)
2. Container is terminated immediately
3. Pod status shows `OOMKilled`
4. Kubernetes restarts container based on `restartPolicy`
...
|--------------|---------|----------|
| **No limits** | Noisy neighbor, node instability | Always set memory limits |
| **Limits = requests = max observed** | Over-provisioning, wasted resources | Set requests at p70, limits at p99 |
| **CPU limits on latency-sensitive apps** | Throttling causes latency spikes | Consider removing CPU limits |
| **Memory limit = JVM heap** | OOMKill from non-heap | Limit = heap + 25% overhead |
| **Same config for all workloads** | Mismatched resources | Profile each workload type |
| **requests >> actual usage** | Scheduling failures, waste | Use VPA recommendations |
...
VPA analyzes historical resource usage and provides recommendations:
...
|------|----------|----------|
| `Off` | Recommendations only | Production analysis |
| `Initial` | Set on pod creation only | Stateful workloads |
| `Recreate` | Evict and recreate pods | Stateless, tolerates restarts |
| `Auto` | Recreate with future in-place support | Future-ready |
...
Goldilocks creates VPAs for all deployments and provides a dashboard:
...
### PromQL Resource Analysis

**CPU Analysis (Target 70-80% utilization)**:
...
**Memory Analysis (Target 80% max utilization)**:
...
|---------------|-------------|-----------|----------------|--------------|
| **API Service** | P70 usage | P99 + 50% or none | P80 usage | P99 + 25% |
...
### JVM Memory Model in Containers

Understanding JVM memory is critical for proper container sizing:
...
|------------|---------------------------|-------------------|------|
| 50% | 512Mi | 512Mi | Under-utilized heap |
| 75% | 768Mi | 256Mi | Optimal balance |
| 80% | 819Mi | 205Mi | Slight OOM risk |
| 90% | 921Mi | 102Mi | High OOM risk |
...
Java 11+ automatically detects container limits with `-XX:+UseContainerSupport` (enabled by default):
...
| Algorithm | Best For | Heap Size | Pause Target | CPU Overhead |
|-----------|----------|-----------|--------------|--------------|
| **G1GC** | General purpose | 4-64GB | 200ms | Medium |
| **ZGC** | Low latency | 8GB-16TB | <10ms | Higher |
| **Shenandoah** | Low latency | Any | <10ms | Higher |
| **ParallelGC** | Throughput | Small-Medium | Not guaranteed | Lower |
| **SerialGC** | Small heaps | <100MB | Not applicable | Lowest |

**G1GC Configuration** (recommended default):
...
JVM GC threads are affected by CPU constraints:
...
**Explicit GC Thread Configuration**:
...
## Python/Node.js Workloads

### Python (Gunicorn) Optimization
...
**automaxprocs for Container-Aware CPU**:
...
| Container overhead | Minimal | 25-40% | 10-20% | 10-20% |
...
Rust has no garbage collector, providing:
...
### Compiled Language Resource Comparison

| Aspect | Go | Rust | C++ |
|--------|-----|------|-----|
| **Memory model** | GC (concurrent) | Ownership (no GC) | Manual |
| **Startup time** | ~50ms | ~10ms | ~10ms |
| **Memory overhead** | Low | Minimal | Minimal |
| **CPU efficiency** | Very High | Highest | Highest |
| **Request sizing** | Actual * 1.2 | Actual * 1.1 | Actual * 1.1 |
| **Container fit** | Excellent | Best | Good |
...
## Resource Optimization in Auto Mode

### Instance Type Bin-Packing

EKS Auto Mode automatically selects instance types based on pending pod requirements:
...
### Over-Provisioning for Quick Scaling

Configure over-provisioning for faster pod scheduling:
...
### Auto Mode Optimization Recommendations

| Scenario | Recommendation | Auto Mode Behavior |
|----------|----------------|-------------------|
| Many small pods | Use smaller instance types | Auto-selects appropriate mix |
| Few large pods | Allow larger instances | Provisions large instances |
| Variable workload | Enable over-provisioning | Maintains buffer capacity |
| Cost optimization | Spot instances | Diversifies across pools |
| Latency-sensitive | Same-AZ placement | Respects topology constraints |

**Best Practices for Auto Mode**:

1. **Set accurate resource requests**: Auto Mode uses requests for scheduling decisions
2. **Avoid excessive limits**: High limits can fragment nodes
3. **Use topology spread**: Distribute pods across zones
4. **Configure PodDisruptionBudgets**: Enable safe consolidation
5. **Monitor bin-packing efficiency**: Track node utilization

---

## Related Documentation

- [Observability Stack Operations](./09-observability-stack.md) - Monitoring and alerting configuration
- [EKS Upgrades](./11-upgrade-operations.md) - Cluster upgrade procedures
